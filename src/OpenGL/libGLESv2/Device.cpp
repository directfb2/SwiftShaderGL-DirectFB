...
Device::Device(sw::Context *context) : Renderer(context, sw::OpenGL, true), context(context)
{
	...
	setFillMode(sw::FILL_SOLID);
	setShadingMode(sw::SHADING_GOURAUD);
	setDepthWriteEnable(true);
	setAlphaTestEnable(false);
	setSourceBlendFactor(sw::BLEND_ONE);
	setDestBlendFactor(sw::BLEND_ZERO);
	setCullMode(sw::CULL_COUNTERCLOCKWISE, true);
	setDepthCompare(sw::DEPTH_LESSEQUAL);
	setAlphaReference(127.5f);
	setAlphaCompare(sw::ALPHA_ALWAYS);
	setAlphaBlendEnable(false);
	setFogEnable(false);
	setSpecularEnable(false);
	setFogColor(0);
	setPixelFogMode(sw::FOG_NONE);
	setFogStart(0.0f);
	setFogEnd(1.0f);
	setFogDensity(1.0f);
	setRangeFogEnable(false);
	setStencilEnable(false);
	setStencilFailOperation(sw::OPERATION_KEEP);
	setStencilZFailOperation(sw::OPERATION_KEEP);
	setStencilPassOperation(sw::OPERATION_KEEP);
	setStencilCompare(sw::STENCIL_ALWAYS);
	setStencilReference(0);
	setStencilMask(0xFFFFFFFF);
	setStencilWriteMask(0xFFFFFFFF);
	setVertexFogMode(sw::FOG_NONE);
	setClipFlags(0);
	setPointSize(1.0f);
	setPointSizeMin(0.125f);
	setPointSizeMax(8192.0f);
	setBlendOperation(sw::BLENDOP_ADD);
	scissorEnable = false;
	setSlopeDepthBias(0.0f);
	setTwoSidedStencil(false);
	setStencilFailOperationCCW(sw::OPERATION_KEEP);
	setStencilZFailOperationCCW(sw::OPERATION_KEEP);
	setStencilPassOperationCCW(sw::OPERATION_KEEP);
	setStencilCompareCCW(sw::STENCIL_ALWAYS);
	setBlendConstant(0xFFFFFFFF);
	setWriteSRGB(false);
	setDepthBias(0.0f);
	setSeparateAlphaBlendEnable(false);
	setSourceBlendFactorAlpha(sw::BLEND_ONE);
	setDestBlendFactorAlpha(sw::BLEND_ZERO);
	setBlendOperationAlpha(sw::BLENDOP_ADD);
	setPointSpriteEnable(true);
	setColorLogicOpEnabled(false);
	setLogicalOperation(sw::LOGICALOP_COPY);

	for(int i = 0; i < 16; i++)
	{
		setAddressingModeU(sw::SAMPLER_PIXEL, i, sw::ADDRESSING_WRAP);
		setAddressingModeV(sw::SAMPLER_PIXEL, i, sw::ADDRESSING_WRAP);
		setAddressingModeW(sw::SAMPLER_PIXEL, i, sw::ADDRESSING_WRAP);
		setBorderColor(sw::SAMPLER_PIXEL, i, 0x00000000);
		setTextureFilter(sw::SAMPLER_PIXEL, i, sw::FILTER_POINT);
		setMipmapFilter(sw::SAMPLER_PIXEL, i, sw::MIPMAP_NONE);
		setMipmapLOD(sw::SAMPLER_PIXEL, i, 0.0f);
	}

	for(int i = 0; i < 4; i++)
	{
		setAddressingModeU(sw::SAMPLER_VERTEX, i, sw::ADDRESSING_WRAP);
		setAddressingModeV(sw::SAMPLER_VERTEX, i, sw::ADDRESSING_WRAP);
		setAddressingModeW(sw::SAMPLER_VERTEX, i, sw::ADDRESSING_WRAP);
		setBorderColor(sw::SAMPLER_VERTEX, i, 0x00000000);
		setTextureFilter(sw::SAMPLER_VERTEX, i, sw::FILTER_POINT);
		setMipmapFilter(sw::SAMPLER_VERTEX, i, sw::MIPMAP_NONE);
	...
	for(int i = 0; i < sw::FRAGMENT_UNIFORM_VECTORS; i++)
	{
	...
	for(int i = 0; i < sw::VERTEX_UNIFORM_VECTORS; i++)
	{
		float zero[4] = {0, 0, 0, 0};
...
void Device::clearColor(float red, float green, float blue, float alpha, unsigned int rgbaMask)
{
	...
	for(int i = 0; i < sw::RENDERTARGETS; ++i)
	{
		if(renderTarget[i])
		{
			...
			clear(rgba, sw::FORMAT_A32B32G32R32F, renderTarget[i], clearRect, rgbaMask);
		}
	}
}
...
void Device::setPixelShader(const sw::PixelShader *pixelShader)
{
...
{
	for(unsigned int i = 0; i < count && startRegister + i < sw::FRAGMENT_UNIFORM_VECTORS; i++)
	{
	...
	pixelShaderConstantsFDirty = std::max(startRegister + count, pixelShaderConstantsFDirty);
	pixelShaderDirty = true;
}
...
void Device::setVertexShader(const sw::VertexShader *vertexShader)
{
...
{
	for(unsigned int i = 0; i < count && startRegister + i < sw::VERTEX_UNIFORM_VECTORS; i++)
	{
	...
	vertexShaderConstantsFDirty = std::max(startRegister + count, vertexShaderConstantsFDirty);
	vertexShaderDirty = true;
}
...
bool Device::stretchRect(sw::Surface *source, const sw::SliceRectF *sourceRect, sw::Surface *dest, const sw::SliceRect *destRect, unsigned char flags)
{
	...
	if(sourceRect)
	{
		...
		{
			sw::swap(sRect.x0, sRect.x1);
		}

		if(sRect.y0 > sRect.y1)
		{
			sw::swap(sRect.y0, sRect.y1);
		}
	}
	else
	{
	...
	if(destRect)
	{
		...
		{
			sw::swap(dRect.x0, dRect.x1);
		}

		if(dRect.y0 > dRect.y1)
		{
			sw::swap(dRect.y0, dRect.y1);
		}
	}
	else
	{
	...
	bool isDepth = (flags & Device::DEPTH_BUFFER) && sw::Surface::isDepth(source->getInternalFormat());
	bool isStencil = (flags & Device::STENCIL_BUFFER) && sw::Surface::isStencil(source->getInternalFormat());
	...
	bool hasQuadLayout = sw::Surface::hasQuadLayout(source->getInternalFormat()) || sw::Surface::hasQuadLayout(dest->getInternalFormat());
	...
	bool smallMargin = sourcePitchB <= source->getWidth() * sw::Surface::bytes(source->getInternalFormat()) + 16;

	if((source->getInternalFormat() == sw::FORMAT_A8R8G8B8 && dest->getInternalFormat() == sw::FORMAT_X8R8G8B8) ||
	   (source->getInternalFormat() == sw::FORMAT_X8R8G8B8 && dest->getInternalFormat() == sw::FORMAT_A8R8G8B8))
	{
		equalFormats = true;
		alpha0xFF = true;
	}
	...
	{
		sw::byte *sourceBuffer = isStencil ? (sw::byte*)source->lockStencil(0, 0, 0, sw::PUBLIC) : (sw::byte*)source->lockInternal(0, 0, 0, sw::LOCK_READONLY, sw::PUBLIC);
		sw::byte *destBuffer = isStencil ? (sw::byte*)dest->lockStencil(0, 0, 0, sw::PUBLIC) : (sw::byte*)dest->lockInternal(0, 0, 0, sw::LOCK_DISCARD, sw::PUBLIC);
	...
	{
		sw::byte *sourceBuffer = (sw::byte*)source->lockInternal((int)sRect.x0, (int)sRect.y0, 0, sw::LOCK_READONLY, sw::PUBLIC);
		sw::byte *destBuffer = (sw::byte*)dest->lockInternal(dRect.x0, dRect.y0, 0, fullCopy ? sw::LOCK_DISCARD : sw::LOCK_WRITEONLY, sw::PUBLIC);

		copyBuffer(sourceBuffer, destBuffer, dRect.width(), dRect.height(), sourcePitchB, destPitchB, sw::Surface::bytes(source->getInternalFormat()), flipX, flipY);
	...
	{
		sw::byte *sourceBytes = (sw::byte*)source->lockInternal((int)sRect.x0, (int)sRect.y0, sourceRect->slice, sw::LOCK_READONLY, sw::PUBLIC);
		sw::byte *destBytes = (sw::byte*)dest->lockInternal(dRect.x0, dRect.y0, destRect->slice, fullCopy ? sw::LOCK_DISCARD : sw::LOCK_WRITEONLY, sw::PUBLIC);

		unsigned int width = dRect.x1 - dRect.x0;
		unsigned int height = dRect.y1 - dRect.y0;

		copyBuffer(sourceBytes, destBytes, width, height, sourcePitchB, destPitchB, sw::Surface::bytes(source->getInternalFormat()), flipX, flipY);
...
bool Device::stretchCube(sw::Surface *source, sw::Surface *dest)
{
	...
	if(!scaling && equalFormats)
	{
		...
		{
			unsigned char *sourceBytes = (unsigned char*)source->lockInternal(0, 0, z, sw::LOCK_READONLY, sw::PUBLIC);
			unsigned char *destBytes = (unsigned char*)dest->lockInternal(0, 0, z, sw::LOCK_READWRITE, sw::PUBLIC);
...
void Device::bindShaderConstants()
{
	if(pixelShaderDirty)
	{
		if(pixelShader)
		{
			...
			pixelShaderConstantsFDirty = pixelShader->dirtyConstantsF;
		}
		else
		{
			setPixelShader(0);
	...
	if(vertexShaderDirty)
	{
		if(vertexShader)
		{
			...
			vertexShaderConstantsFDirty = vertexShader->dirtyConstantsF;
		}
		else
		{
			setVertexShader(0);
...
bool Device::bindViewport()
{
	...
	else
	{
		...
		for(int i = 0; i < sw::RENDERTARGETS; ++i)
		{
			if(renderTarget[i])
			{
				scissor.x0 = std::max(scissor.x0, 0);
				scissor.x1 = std::min(scissor.x1, renderTarget[i]->getWidth());
				scissor.y0 = std::max(scissor.y0, 0);
				scissor.y1 = std::min(scissor.y1, renderTarget[i]->getHeight());
			}
		}
		...
		{
			scissor.x0 = std::max(scissor.x0, 0);
			scissor.x1 = std::min(scissor.x1, depthBuffer->getWidth());
			scissor.y0 = std::max(scissor.y0, 0);
			scissor.y1 = std::min(scissor.y1, depthBuffer->getHeight());
		}

		if(stencilBuffer)
		{
			scissor.x0 = std::max(scissor.x0, 0);
			scissor.x1 = std::min(scissor.x1, stencilBuffer->getWidth());
			scissor.y0 = std::max(scissor.y0, 0);
			scissor.y1 = std::min(scissor.y1, stencilBuffer->getHeight());
		}

		// Ensure scissor range is positive.
		scissor.x0 = std::max(scissor.x0, 0);
		scissor.x1 = std::max(scissor.x1, 0);
		scissor.y0 = std::max(scissor.y0, 0);
		scissor.y1 = std::max(scissor.y1, 0);
