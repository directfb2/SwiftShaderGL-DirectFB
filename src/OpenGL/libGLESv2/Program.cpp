...
// Returns the index of the texture image unit (0-19) corresponding to
// a sampler index (0-15 for the pixel shader and 0-3 for the vertex shader)
...
// Returns the texture type for a given sampler type and index
// (0-15 for the pixel shader and 0-3 for the vertex shader)
...
bool Program::setUniformfv(GLint location, GLsizei count, const GLfloat *v, int numElements)
{
	...
	{
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLfloat)* numElements, v, numElements * sizeof(GLfloat) * count);
	}
...
bool Program::setUniform1iv(GLint location, GLsizei count, const GLint *v)
{
	...
	{
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLint), v, sizeof(GLint) * count);
	}
	else if(targetUniform->type == GL_BOOL)
	{
		...
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLboolean), boolParams, sizeof(GLboolean) * count);
...
bool Program::setUniformiv(GLint location, GLsizei count, const GLint *v, int numElements)
{
	...
	{
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLint)* numElements, v, numElements * sizeof(GLint)* count);
	}
	else if(targetUniform->type == boolType[index])
	{
		...
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLboolean)* numElements, boolParams, numElements * sizeof(GLboolean) * count);
...
bool Program::setUniform1uiv(GLint location, GLsizei count, const GLuint *v)
{
	...
	{
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLuint), v, sizeof(GLuint) * count);
	}
	else if(targetUniform->type == GL_BOOL)
	{
		GLboolean *boolParams = new GLboolean[count];
...
bool Program::setUniformuiv(GLint location, GLsizei count, const GLuint *v, int numElements)
{
	...
	{
		memcpy(targetUniform->data + uniformIndex[location].element * sizeof(GLuint)* numElements, v, numElements * sizeof(GLuint)* count);
	}
	else if(targetUniform->type == boolType[index])
	{
		GLboolean *boolParams = new GLboolean[count * numElements];
...
bool Program::getUniformfv(GLint location, GLsizei *bufSize, GLfloat *params)
{
	...
	case GL_BOOL:
		...
		break;
	case GL_FLOAT:
		memcpy(params, targetUniform->data + uniformIndex[location].element * count * sizeof(GLfloat), count * sizeof(GLfloat));
		break;
	case GL_INT:
		{
...
bool Program::getUniformiv(GLint location, GLsizei *bufSize, GLint *params)
{
	...
	case GL_INT:
	case GL_UNSIGNED_INT:
		memcpy(params, targetUniform->data + uniformIndex[location].element * count * sizeof(GLint), count * sizeof(GLint));
		break;
	default:
		UNREACHABLE(targetUniform->type);
		break;
...
bool Program::getUniformuiv(GLint location, GLsizei *bufSize, GLuint *params)
{
	...
	case GL_INT:
	case GL_UNSIGNED_INT:
		memcpy(params, targetUniform->data + uniformIndex[location].element * count * sizeof(GLuint), count * sizeof(GLuint));
		break;
	default:
		UNREACHABLE(targetUniform->type);
		break;
...
void Program::applyUniformBuffers(Device *device, BufferBinding *uniformBuffers)
{
	...
	for(unsigned int bufferBindingIndex = 0; bufferBindingIndex < MAX_UNIFORM_BUFFER_BINDINGS; bufferBindingIndex++)
	{
		...
		device->VertexProcessor::setUniformBuffer(bufferBindingIndex, vsBuffer ? vsBuffer->getResource() : nullptr, (index != -1) ? uniformBuffers[index].getOffset() : 0);
		index = fragmentUniformBuffers[bufferBindingIndex];
		Buffer *psBuffer = (index != -1) ? (Buffer*)uniformBuffers[index].get() : nullptr;
		device->PixelProcessor::setUniformBuffer(bufferBindingIndex, psBuffer ? psBuffer->getResource() : nullptr, (index != -1) ? uniformBuffers[index].getOffset() : 0);
	}
}
...
void Program::applyTransformFeedback(Device *device, TransformFeedback *transformFeedback)
{
	...
	ASSERT(std::max<int>(MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS, sw::MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS) <= 64);
	...
	case GL_SEPARATE_ATTRIBS:
		{
			maxVaryings = std::min(maxVaryings, (unsigned int)MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS);
			...
			for(unsigned int index = 0; index < maxVaryings; ++index)
			{
				...
				device->VertexProcessor::setTransformFeedbackBuffer(index, transformFeedbackBuffers[index].get()->getResource(), transformFeedbackBuffers[index].getOffset() + baseOffset, transformFeedbackLinkedVaryings[index].reg * 4 + transformFeedbackLinkedVaryings[index].col, nbRegs, nbComponentsPerReg, componentStride);
	...
	case GL_INTERLEAVED_ATTRIBS:
		{
			...
			maxVaryings = std::min(maxVaryings, (unsigned int)sw::MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS);
			...
			for(unsigned int index = 0; index < maxVaryings; ++index)
			{
				...
				int nbRegs = rowCount > 1 ? colCount * size : size;
				int nbComponentsPerReg = rowCount > 1 ? rowCount : colCount;
				device->VertexProcessor::setTransformFeedbackBuffer(index, resource, baseOffset + (totalComponents * sizeof(float)), transformFeedbackLinkedVaryings[index].reg * 4 + transformFeedbackLinkedVaryings[index].col, nbRegs, nbComponentsPerReg, componentStride);
	...
	for(auto const &input : psVaryings)
	{
		...
		for(auto const &output : vsVaryings)
		{
			if(output.name == input.name)
			{
				...
				{
					appendToInfoLog("Type of vertex varying %s does not match that of the fragment varying",
					                output.name.c_str());
					return false;
				}
				...
				{
					appendToInfoLog("Interpolation qualifiers for %s differ between vertex and fragment shaders",
					                output.name.c_str());
					return false;
				}
		...
		if(!matched)
		{
			...
			{
				appendToInfoLog("Fragment varying %s does not match any vertex varying",
				                input.name.c_str());
				return false;
			}
		}
	...
	for(const std::string &indexedTfVaryingName : transformFeedbackVaryings)
	{
		...
		for(const glsl::Varying& varying : vertexShader->varyings)
		{
			if(tfVaryingName == varying.name)
			{
				...
				{
					appendToInfoLog("Two transform feedback varyings specify the same output variable (%s)",
					                indexedTfVaryingName.c_str());
					return false;
				}
				...
				{
					appendToInfoLog("Specified transform feedback varying index out of bounds (%s)",
					                indexedTfVaryingName.c_str());
					return false;
				}
		...
		if(!found)
		{
			appendToInfoLog("Transform feedback varying %s does not exist in the vertex shader",
			                tfVaryingName.c_str());
			return false;
		}
	}
...
void Program::link()
{
	...
	// Link uniform blocks before uniforms to make it easy to assign block indices
	// to fields
...
bool Program::linkAttributes()
{
	...
	// Link attributes that have an API provided binding location
	// but no GLSL layout location
	for(auto const &attribute : vertexShader->activeAttributes)
	{
...
bool Program::linkAttribute(const glsl::Attribute &attribute, int location, unsigned int &usedLocations)
{
	...
	if(location == -1)
	{
		...
		{
			appendToInfoLog("Too many active attributes (%s)",
	...
	else
	{
		...
		{
			appendToInfoLog("Active attribute (%s) at location %d is too big to fit",
			                attribute.name.c_str(), location);
			return false;
		}
		...
		if(vertexShader->getShaderVersion() >= 300)
		{
			for(auto const &previousAttrib : linkedAttribute)
			{
				...
				{
					appendToInfoLog("Attribute '%s' aliases attribute '%s' at location %d",
					                attribute.name.c_str(), previousAttrib.name.c_str(), location);
					return false;
				}
				...
				{
					appendToInfoLog("Attribute '%s' aliases attribute '%s' at location %d",
					                attribute.name.c_str(), previousAttrib.name.c_str(), previousLocation);
					return false;
				}
			}
		}
	...
	for(const auto &uniform : shader->activeUniforms)
	{
		...
		if(uniform.blockId >= 0)
		{
			...
			if(activeUniformBlocks[uniform.blockId].dataSize > MAX_UNIFORM_BLOCK_SIZE)
			{
				if(shader->getType() == GL_VERTEX_SHADER)
				{
					appendToInfoLog("Vertex shader active uniform block (%s) exceeds GL_MAX_UNIFORM_BLOCK_SIZE (%d)",
					                uniformBlockName.c_str(), MAX_UNIFORM_BLOCK_SIZE);
					return false;
				}
				else if(shader->getType() == GL_FRAGMENT_SHADER)
				{
					appendToInfoLog("Fragment shader active uniform block (%s) exceeds GL_MAX_UNIFORM_BLOCK_SIZE (%d)",
					                uniformBlockName.c_str(), MAX_UNIFORM_BLOCK_SIZE);
					return false;
				}
				else UNREACHABLE(shader->getType());
...
bool Program::defineUniform(GLenum shader, const glsl::Uniform &glslUniform, const Uniform::BlockInfo& blockInfo)
{
	if(IsSamplerUniform(glslUniform.type))
	{
		...
		do
		{
			if(shader == GL_VERTEX_SHADER)
			{
				...
				{
					appendToInfoLog("Vertex shader sampler count exceeds MAX_VERTEX_TEXTURE_IMAGE_UNITS (%d)",
			...
			else if(shader == GL_FRAGMENT_SHADER)
			{
				...
				{
					appendToInfoLog("Pixel shader sampler count exceeds MAX_TEXTURE_IMAGE_UNITS (%d)",
	...
	else
	{
		...
		{
			appendToInfoLog("Types for uniform %s do not match between the vertex and fragment shader",
			                uniform->name.c_str());
			return false;
		}
		...
		{
			appendToInfoLog("Precisions for uniform %s do not match between the vertex and fragment shader",
			                uniform->name.c_str());
			return false;
		}
	...
	if(uniform->blockInfo.index < 0)
	{
		if(shader == GL_VERTEX_SHADER)
		{
			...
			{
				appendToInfoLog("Vertex shader active uniforms exceed GL_MAX_VERTEX_UNIFORM_VECTORS (%d)",
		...
		else if(shader == GL_FRAGMENT_SHADER)
		{
			...
			{
				appendToInfoLog("Fragment shader active uniforms exceed GL_MAX_FRAGMENT_UNIFORM_VECTORS (%d)",
...
bool Program::areMatchingUniformBlocks(const glsl::UniformBlock &block1, const glsl::UniformBlock &block2, const Shader *shader1, const Shader *shader2)
{
	...
	{
		appendToInfoLog("Types for interface block '%s' differ between vertex and fragment shaders",
		                block1.name.c_str());
		return false;
	}
	...
	{
		appendToInfoLog("Array sizes differ for interface block '%s' between vertex and fragment shaders",
		                block1.name.c_str());
		return false;
	}
	...
	{
		appendToInfoLog("Layout qualifiers differ for interface block '%s' between vertex and fragment shaders",
		                block1.name.c_str());
		return false;
	}
	...
	for(size_t blockMemberIndex = 0; blockMemberIndex < numBlockMembers; blockMemberIndex++)
	{
		...
		{
			appendToInfoLog("Array sizes for %s differ between vertex and fragment shaders",
			                member1.name.c_str());
			return false;
		}
		...
		{
			appendToInfoLog("Precisions for %s differ between vertex and fragment shaders",
			                member1.name.c_str());
			return false;
		}
		...
		{
			appendToInfoLog("Types for %s differ between vertex and fragment shaders",
			                member1.name.c_str());
			return false;
		}
		...
		{
			appendToInfoLog("Matrix packings for %s differ between vertex and fragment shaders",
			                member1.name.c_str());
			return false;
		}
	}
...
bool Program::areMatchingFields(const std::vector<glsl::ShaderVariable>& fields1, const std::vector<glsl::ShaderVariable>& fields2, const std::string& name)
{
	...
	{
		appendToInfoLog("Structure lengths for %s differ between vertex and fragment shaders",
		                name.c_str());
		return false;
	}
	...
	for(size_t i = 0; i < fields1.size(); ++i)
	{
		...
		{
			appendToInfoLog("Type for %s.%s differ between vertex and fragment shaders",
			                name.c_str(), fields1[i].name.c_str());
			return false;
		}

		if(fields1[i].arraySize != fields2[i].arraySize)
		{
			appendToInfoLog("Array size for %s.%s differ between vertex and fragment shaders",
			                name.c_str(), fields1[i].name.c_str());
			return false;
		}
...
bool Program::linkUniformBlocks(const Shader *vertexShader, const Shader *fragmentShader)
{
	...
	// Check that interface blocks defined in the vertex and fragment shaders
	// are identical
...
{
	// If any two active samplers in a program are of different types, but refer
	// to the same texture image unit, and this is the current program, then
	// ValidateProgram will fail, and DrawArrays and DrawElements will issue the
	...
	for(unsigned int i = 0; i < MAX_TEXTURE_IMAGE_UNITS; i++)
	{
		if(samplersPS[i].active)
		{
			...
			if(unit >= MAX_COMBINED_TEXTURE_IMAGE_UNITS)
			{
				if(logErrors)
				{
					appendToInfoLog("Sampler uniform (%d) exceeds MAX_COMBINED_TEXTURE_IMAGE_UNITS (%d)",
					                unit, MAX_COMBINED_TEXTURE_IMAGE_UNITS);
				}
			...
			if(textureUnitType[unit] != TEXTURE_UNKNOWN)
			{
				if(samplersPS[i].textureType != textureUnitType[unit])
				{
					if(logErrors)
					{
						appendToInfoLog("Samplers of conflicting types refer to the same texture image unit (%d)",
						                unit);
					}
