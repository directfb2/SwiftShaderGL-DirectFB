...
void GL_APIENTRY ActiveTexture(GLenum texture)
{
	...
	if(context)
	{
		...
		{
			return es2::error(GL_INVALID_ENUM);
		}
...
void GL_APIENTRY AttachShader(GLuint program, GLuint shader)
{
	...
	if(context)
	{
		...
		if(!programObject)
		{
			...
			{
				return es2::error(GL_INVALID_OPERATION);
			}
			else
			{
				return es2::error(GL_INVALID_VALUE);
			}
		}
...
void GL_APIENTRY BindTexture(GLenum target, GLuint texture)
{
	...
	if(context)
	{
		...
		{
		case GL_TEXTURE_2D:
			context->bindTexture(es2::TEXTURE_2D, texture);
			break;
		case GL_TEXTURE_CUBE_MAP:
			context->bindTexture(es2::TEXTURE_CUBE, texture);
			break;
		case GL_TEXTURE_EXTERNAL_OES:
			context->bindTexture(es2::TEXTURE_EXTERNAL, texture);
			break;
		case GL_TEXTURE_2D_ARRAY:
			context->bindTexture(es2::TEXTURE_2D_ARRAY, texture);
			break;
		case GL_TEXTURE_3D:
			context->bindTexture(es2::TEXTURE_3D, texture);
			break;
		default:
			return es2::error(GL_INVALID_ENUM);
		}
...
void GL_APIENTRY BlendColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha)
{
	...
	if(context)
	{
		context->setBlendColor(sw::clamp01(red), sw::clamp01(green), sw::clamp01(blue), sw::clamp01(alpha));
	}
}
...
{
	size = static_cast<GLint>(size); // Workaround

	TRACE("(GLenum target = 0x%X, GLsizeiptr size = %d, const GLvoid *data = %p, GLenum usage = %d)", target, size, data, usage);
...
{
	size = static_cast<GLint>(size); // Workaround
	offset = static_cast<GLint>(offset); // Workaround

	TRACE("(GLenum target = 0x%X, GLintptr offset = %d, GLsizeiptr size = %d, const GLvoid *data = %p)", target, offset, size, data);
...
GLenum GL_APIENTRY CheckFramebufferStatus(GLenum target)
{
	...
	if(target != GL_FRAMEBUFFER && target != GL_DRAW_FRAMEBUFFER && target != GL_READ_FRAMEBUFFER)
	{
		return es2::error(GL_INVALID_ENUM, 0);
	}
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d, GLint border = %d, GLsizei imageSize = %d, const GLvoid *data = %p)", target, level, internalformat, width, height, border, imageSize, data);

	if(!es2::validImageSize(level, width, height) || border != 0 || imageSize < 0)
	{
		return es2::error(GL_INVALID_VALUE);
	}

	if(!es2::IsCompressed(internalformat))
	{
		return es2::error(GL_INVALID_ENUM);
	}
	...
	if(context)
	{
		...
		{
			return es2::error(validationError);
		}
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLsizei width = %d, GLsizei height = %d, GLenum format = 0x%X, GLsizei imageSize = %d, const GLvoid *data = %p)", target, level, xoffset, yoffset, width, height, format, imageSize, data);
	...
	if(xoffset < 0 || yoffset < 0 || !es2::validImageSize(level, width, height) || imageSize < 0)
	{
		return es2::error(GL_INVALID_VALUE);
	}

	if(!es2::IsCompressed(format))
	{
		return es2::error(GL_INVALID_ENUM);
	}
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLenum internalformat = 0x%X, GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d, GLint border = %d)", target, level, internalformat, x, y, width, height, border);

	if(!es2::validImageSize(level, width, height))
	{
		return es2::error(GL_INVALID_VALUE);
	}
	...
	if(context)
	{
		...
		{
			return es2::error(GL_INVALID_FRAMEBUFFER_OPERATION);
		}
		...
		if(gl::IsUnsizedInternalFormat(internalformat))
		{
			...
			else if(es2::GetColorComponentType(colorbufferFormat) == GL_UNSIGNED_NORMALIZED && es2::GetRedSize(colorbufferFormat) <= 8)
			{
				internalformat = gl::GetSizedInternalFormat(internalformat, GL_UNSIGNED_BYTE);
			}
			else if(es2::GetColorComponentType(colorbufferFormat) == GL_INT)
			{
				internalformat = gl::GetSizedInternalFormat(internalformat, GL_INT);
			}
			else if(es2::GetColorComponentType(colorbufferFormat) == GL_UNSIGNED_INT)
			{
				internalformat = gl::GetSizedInternalFormat(internalformat, GL_UNSIGNED_INT);
			}
			else if(es2::GetColorComponentType(colorbufferFormat) == GL_FLOAT && es2::GetRedSize(colorbufferFormat) == 16)
			{
				internalformat = gl::GetSizedInternalFormat(internalformat, GL_HALF_FLOAT_OES);
			}
			else if(es2::GetColorComponentType(colorbufferFormat) == GL_FLOAT && es2::GetRedSize(colorbufferFormat) == 32)
			{
		...
		if(!es2::ValidateCopyFormats(internalformat, colorbufferFormat))
		{
			return;
		}
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d)", target, level, xoffset, yoffset, x, y, width, height);
...
{
	TRACE("(GLenum mode = 0x%X, GLsizei count = %d, GLenum type = 0x%X, const GLvoid *indices = %p)", mode, count, type, indices);
	...
	if(context)
	{
		...
		context->drawElements(mode, 0, es2::MAX_ELEMENT_INDEX, count, type, indices);
	}
}

void GL_APIENTRY DrawArraysInstancedEXT(GLenum mode, GLint first, GLsizei count, GLsizei instanceCount)
{
	TRACE("(GLenum mode = 0x%X, GLint first = %d, GLsizei count = %d, GLsizei instanceCount = %d)", mode, first, count, instanceCount);
...
{
	TRACE("(GLenum mode = 0x%X, GLsizei count = %d, GLenum type = 0x%X, const void *indices = %p, GLsizei instanceCount = %d)", mode, count, type, indices, instanceCount);
	...
	if(context)
	{
		...
		context->drawElements(mode, 0, es2::MAX_ELEMENT_INDEX, count, type, indices, instanceCount);
	}
}
...
void GL_APIENTRY FramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer)
{
	...
	if(context)
	{
		...
		default:
			...
			if((attachment - GL_COLOR_ATTACHMENT0) >= es2::MAX_COLOR_ATTACHMENTS)
			{
				return es2::error(GL_INVALID_OPERATION);
			}
...
void GL_APIENTRY GenerateMipmap(GLenum target)
{
	...
	if(context)
	{
		...
		if(!es2::IsMipmappable(texture->getFormat(target, texture->getBaseLevel())))
		{
			return es2::error(GL_INVALID_OPERATION);
		}
		...
		{
			es2::TextureCubeMap *cube = context->getTextureCubeMap();
...
{
	TRACE("(GLuint program = %d, GLuint index = %d, GLsizei bufsize = %d, GLsizei *length = %p, GLint *size = %p, GLenum *type = %p, GLchar *name = %p)", program, index, bufsize, length, size, type, name);
...
{
	TRACE("(GLuint program = %d, GLuint index = %d, GLsizei bufsize = %d, GLsizei *length = %p, GLint *size = %p, GLenum *type = %p, GLchar *name = %s)", program, index, bufsize, length, size, type, name);
...
{
	TRACE("(GLuint program = %d, GLsizei maxcount = %d, GLsizei *count = %p, GLuint *shaders = %p)", program, maxcount, count, shaders);
...
int GL_APIENTRY GetAttribLocation(GLuint program, const GLchar *name)
{
	...
	if(context)
	{
		...
		if(!programObject)
		{
			if(context->getShader(program))
			{
				return es2::error(GL_INVALID_OPERATION, -1);
			}
			else
			{
				return es2::error(GL_INVALID_VALUE, -1);
			}
		}
...
{
	TRACE("(GLenum target = 0x%X, GLenum attachment = 0x%X, GLenum pname = 0x%X, GLint *params = %p)", target, attachment, pname, params);
	...
	if(context)
	{
		...
		es2::Renderbuffer *renderbuffer = nullptr;
		switch(attachment)
		{
		case GL_BACK:
		...
		case GL_DEPTH_STENCIL_ATTACHMENT:
			...
			break;
		default:
			ASSERT((unsigned int)(attachment - GL_COLOR_ATTACHMENT0) < es2::MAX_COLOR_ATTACHMENTS);
		...
		GLenum attachmentObjectType = GL_NONE;
		if(framebufferName == 0)
		{
			attachmentObjectType = GL_FRAMEBUFFER_DEFAULT;
		}
		else if(attachmentType == GL_NONE || es2::Framebuffer::IsRenderbuffer(attachmentType))
		{
		...
		if(attachmentObjectType != GL_NONE)
		{
			...
			case GL_FRAMEBUFFER_ATTACHMENT_COMPONENT_TYPE:
				...
				*params = es2::GetComponentType(renderbuffer->getFormat(), attachment);
				break;
			case GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING:
				*params = es2::GetColorEncoding(renderbuffer->getFormat());
				break;
			default:
				return es2::error(GL_INVALID_ENUM);
			}
...
void GL_APIENTRY GetIntegerv(GLenum pname, GLint *params)
{
	...
	if(!context)
	{
		...
		// Workaround when querying these values before creating a context.
		switch(pname)
		{
		...
		if(!(context->getIntegerv(pname, params)))
		{
			...
			else if(nativeType == GL_FLOAT)
			{
				...
				for(unsigned int i = 0; i < numParams; ++i)
				{
					...
					{
						params[i] = sw::convert_float_fixed(floatParams[i]);
					}
					else
					{
...
{
	TRACE("(GLuint program = %d, GLsizei bufsize = %d, GLsizei *length = %p, GLchar *infolog = %p)", program, bufsize, length, infolog);
...
{
	TRACE("(GLuint shader = %d, GLsizei bufsize = %d, GLsizei *length = %p, GLchar *infolog = %p)", shader, bufsize, length, infolog);
...
{
	TRACE("(GLenum shadertype = 0x%X, GLenum precisiontype = 0x%X, GLint *range = %p, GLint *precision = %p)", shadertype, precisiontype, range, precision);
...
{
	TRACE("(GLuint shader = %d, GLsizei bufsize = %d, GLsizei *length = %p, GLchar *source = %p)", shader, bufsize, length, source);
...
const GLubyte *GL_APIENTRY GetString(GLenum name)
{
	...
	default:
		return es2::error(GL_INVALID_ENUM, (GLubyte*)nullptr);
	}
}
...
{
	TRACE("(GLuint program = %d, GLint location = %d, GLsizei bufSize = %d, GLfloat *params = %p)", program, location, bufSize, params);
...
{
	TRACE("(GLuint program = %d, GLint location = %d, GLsizei bufSize = %d, GLint *params = %p)", program, location, bufSize, params);
...
void GL_APIENTRY GetVertexAttribfv(GLuint index, GLenum pname, GLfloat *params)
{
	...
	if(context)
	{
		...
		case GL_CURRENT_VERTEX_ATTRIB:
			{
				const es2::VertexAttribute& attrib = context->getCurrentVertexAttributes()[index];
				for(int i = 0; i < 4; ++i)
				{
...
void GL_APIENTRY Hint(GLenum target, GLenum mode)
{
	...
	if(context)
	{
		...
		case GL_FRAGMENT_SHADER_DERIVATIVE_HINT_OES:
			...
			break;
		case es2::GL_TEXTURE_FILTERING_HINT_CHROMIUM:
...
GLboolean GL_APIENTRY IsEnabled(GLenum cap)
{
	...
	if(context)
	{
		...
		default:
			return es2::error(GL_INVALID_ENUM, false);
		}
	}
...
void GL_APIENTRY ReadnPixelsEXT(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLsizei bufSize, GLvoid *data)
{
	TRACE("(GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d, GLenum format = 0x%X, GLenum type = 0x%X, GLsizei bufSize = 0x%d, GLvoid *data = %p)", x, y, width, height, format, type, bufSize, data);
...
{
	TRACE("(GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d, GLenum format = 0x%X, GLenum type = 0x%X, GLvoid *pixels = %p)", x, y, width, height, format, type,  pixels);
...
{
	TRACE("(GLenum target = 0x%X, GLsizei samples = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d)", target, samples, internalformat, width, height);
	...
	if(samples > es2::IMPLEMENTATION_MAX_SAMPLES ||
	   (es2::IsNonNormalizedInteger(internalformat) && samples > 0))
	{
		return es2::error(GL_INVALID_OPERATION);
	}
...
void GL_APIENTRY SampleCoverage(GLclampf value, GLboolean invert)
{
	...
	if(context)
	{
		context->setSampleCoverageParams(sw::clamp01(value), invert != GL_FALSE);
	}
}
...
{
	TRACE("(GLsizei n = %d, const GLuint *shaders = %p, GLenum binaryformat = 0x%X, const GLvoid *binary = %p, GLsizei length = %d)", n, shaders, binaryformat, binary, length);
...
{
	TRACE("(GLuint shader = %d, GLsizei count = %d, const GLchar *const*string = %p, const GLint *length = %p)", shader, count, string, length);
...
GLboolean GL_APIENTRY TestFenceNV(GLuint fence)
{
	...
	if(context)
	{
		...
		if(!fenceObject)
		{
			return es2::error(GL_INVALID_OPERATION, GL_TRUE);
		}
...
void GL_APIENTRY TexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *data)
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint internalformat = %d, GLsizei width = %d, GLsizei height = %d, GLint border = %d, GLenum format = 0x%X, GLenum type = 0x%X, const GLvoid *data =  %p)", target, level, internalformat, width, height, border, format, type, data);
	...
	if(context)
	{
		...
		GLenum validationError = es2::ValidateTextureFormatType(format, type, internalformat, target);
		if(validationError != GL_NO_ERROR)
		{
			return es2::error(validationError);
		}
...
void GL_APIENTRY TexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid *data)
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLsizei width = %d, GLsizei height = %d, GLenum format = 0x%X, GLenum type = 0x%X, const GLvoid *data = %p)", target, level, xoffset, yoffset, width, height, format, type, data);
...
{
	TRACE("(GLuint index = %d, GLint size = %d, GLenum type = 0x%X, GLboolean normalized = %d, GLsizei stride = %d, const GLvoid *ptr = %p)", index, size, type, normalized, stride, ptr);
...
{
	TRACE("(GLint srcX0 = %d, GLint srcY0 = %d, GLint srcX1 = %d, GLint srcY1 = %d, GLint dstX0 = %d, GLint dstY0 = %d, GLint dstX1 = %d, GLint dstY1 = %d, GLbitfield mask = 0x%X, GLenum filter = 0x%X)", srcX0, srcY0, srcX1, srcX1, dstX0, dstY0, dstX1, dstY1, mask, filter);
...
void GL_APIENTRY TexImage3DOES(GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLenum format, GLenum type, const GLvoid *data)
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d, GLint border = %d, GLenum format = 0x%X, GLenum type = 0x%X, const GLvoid *data = %p)", target, level, internalformat, width, height, depth, border, format, type, data);
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLint zoffset = %d, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d, GLenum format = 0x%X, GLenum type = 0x%X, const GLvoid *data = %p)", target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, data);
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLint zoffset = %d, GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d)", target, level, xoffset, yoffset, zoffset, x, y, width, height);
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d, GLint border = %d, GLsizei imageSize = %d, const GLvoid *data = %p)", target, level, internalformat, width, height, depth, border, imageSize, data);
...
{
	TRACE("(GLenum target = 0x%X, GLint level = %d, GLint xoffset = %d, GLint yoffset = %d, GLint zoffset = %d, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d, GLenum format = 0x%X, GLsizei imageSize = %d, const void *data = %p)", target, level, xoffset, yoffset, zoffset, width, height, depth, format, imageSize, data);
	...
	if(xoffset < 0 || yoffset < 0 || zoffset < 0 || !es2::validImageSize(level, width, height) || depth < 0 || imageSize < 0)
	{
		return es2::error(GL_INVALID_VALUE);
	}
...
{
	TRACE("(GLenum target = 0x%X, GLenum attachment = 0x%X, GLenum textarget = 0x%X, GLuint texture = %d, GLint level = %d, GLint zoffset = %d)", target, attachment, textarget, texture, level, zoffset);
			...
			case GL_COLOR_ATTACHMENT15_EXT:
				{
					...
					if(index >= es2::MAX_COLOR_ATTACHMENTS)
					{
						return es2::error(GL_INVALID_OPERATION);
					}
					if(index != i)
