...
{
	// Number of columns for matrices or 1 for scalars and vectors
	return (VariableRowCount(type) > 1) ? VariableColumnCount(type) : 1;
}

int VariableRegisterSize(GLenum type)
{
	// Number of rows for matrices or columns for scalars and vectors
...
GLenum ValidateSubImageParams(bool compressed, bool copy, GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, Texture *texture)
{
	if(!texture)
	{
...
GLenum ValidateSubImageParams(bool compressed, bool copy, GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width, GLsizei height, GLsizei depth, GLenum format, GLenum type, Texture *texture)
{
	if(!texture)
	{
		return GL_INVALID_OPERATION;
...
bool ValidateCopyFormats(GLenum textureFormat, GLenum colorbufferFormat)
{
	...
	case GL_ALPHA:
		if(baseColorbufferFormat != GL_ALPHA &&
		   ...
		   baseColorbufferFormat != GL_BGRA_EXT)
		{
			return error(GL_INVALID_OPERATION, false);
		}
		break;
...
{
	return target == GL_TEXTURE_2D || IsCubemapTextureTarget(target) || target == GL_TEXTURE_2D_ARRAY;
}
...
bool FloatFitsInInt(float f)
{
	...
	//
	// So instead subtract an ULP from "(float) INT32_MAX", cast that
	// to an int, and do the comparison with that value.
	// That value is 2147483520, and can be found with the following code:
