...
bool ValidateTexParamParameters(GLenum pname, GLint param)
{
	...
	case GL_TEXTURE_MAX_ANISOTROPY_EXT:
		if(param < 1)
		{
			return es2::error(GL_INVALID_VALUE, false);
		}
		return true;
...
{
	TRACE("(GLenum mode = 0x%X, GLuint start = %d, GLuint end = %d, GLsizei count = %d, GLenum type = 0x%X, const void *indices = %p)", mode, start, end, count, type, indices);
...
void GL_APIENTRY CopyTexSubImage3D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLint x, GLint y, GLsizei width, GLsizei height)
{
	...
	if(context)
	{
		...
		if(!es2::ValidateCopyFormats(textureFormat, colorbufferFormat))
		{
			return;
		}
...
{
	TRACE("(GLuint id = %d, GLenum pname = 0x%X, GLint *params = %p)", id, pname, params);
...
GLboolean GL_APIENTRY UnmapBuffer(GLenum target)
{
	...
	if(context)
	{
		...
		{
			return es2::error(GL_INVALID_ENUM, GL_TRUE);
		}
...
{
	TRACE("(GLint srcX0 = %d, GLint srcY0 = %d, GLint srcX1 = %d, GLint srcY1 = %d, GLint dstX0 = %d, GLint dstY0 = %d, GLint dstX1 = %d, GLint dstY1 = %d, GLbitfield mask = 0x%X, GLenum filter = 0x%X)", srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter);
...
{
	TRACE("(GLenum target = 0x%X, GLenum attachment = 0x%X, GLuint texture = %d, GLint level = %d, GLint layer = %d)", target, attachment, texture, level, layer);
...
{
	TRACE("(GLenum target = 0x%X,  GLintptr offset = %d, GLsizeiptr length = %d, GLbitfield access = %X)", target, offset, length, access);

	if((offset < 0) || (length < 0))
	{
		return es2::error(GL_INVALID_VALUE, nullptr);
	}

	if(!(access & (GL_MAP_READ_BIT | GL_MAP_WRITE_BIT)))
	{
		return es2::error(GL_INVALID_OPERATION, nullptr);
	}
	else if((access & GL_MAP_READ_BIT) && (access & (GL_MAP_INVALIDATE_RANGE_BIT |
	...
	if(context)
	{
		...
		if(!context->getBuffer(target, &buffer))
		{
			return es2::error(GL_INVALID_ENUM, nullptr);
		}
...
{
	TRACE("(GLenum target = 0x%X,  GLintptr offset = %d, GLsizeiptr length = %d)", target, offset, length);
...
{
	TRACE("(GLenum target = 0x%X, GLuint index = %d, GLint *data = %p)", target, index, data);
		   ...
		   !context->getIntegerv(target, data))
		{
			...
			else if(nativeType == GL_FLOAT)
			{
				...
				for(unsigned int i = 0; i < numParams; ++i)
				{
					...
					{
						data[i] = sw::convert_float_fixed(floatParams[i]);
					}
					else
					{
...
{
	TRACE("(GLenum target = 0x%X, GLuint index = %d, GLuint buffer = %d, GLintptr offset = %d, GLsizeiptr size = %d)", target, index, buffer, offset, size);
	...
	if(context)
	{
		...
		case GL_TRANSFORM_FEEDBACK_BUFFER:
			if(index >= es2::MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS)
			{
				return es2::error(GL_INVALID_VALUE);
			}
		...
		case GL_UNIFORM_BUFFER:
			if(index >= es2::MAX_UNIFORM_BUFFER_BINDINGS)
			{
				return es2::error(GL_INVALID_VALUE);
			}
			if(offset % es2::UNIFORM_BUFFER_OFFSET_ALIGNMENT != 0)
			{
				return es2::error(GL_INVALID_VALUE);
			}
...
{
	TRACE("(GLenum target = 0x%X, GLuint index = %d, GLuint buffer = %d)", target, index, buffer);
...
{
	TRACE("(GLuint program = %d, GLsizei count = %d, const GLchar *const*varyings = %p, GLenum bufferMode = 0x%X)", program, count, varyings, bufferMode);

	switch(bufferMode)
	{
	case GL_SEPARATE_ATTRIBS:
		if(count > es2::MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS)
		{
			return es2::error(GL_INVALID_VALUE);
		}
...
{
	TRACE("(GLuint program = %d, GLuint index = %d, GLsizei bufSize = %d, GLsizei *length = %p, GLsizei *size = %p, GLenum *type = %p, GLchar *name = %p)", program, index, bufSize, length, size, type, name);
...
{
	TRACE("(GLuint program = %d, GLuint index = %d, GLsizei bufSize = %d, GLsizei *length = %p, GLsizei *size = %p, GLenum *type = %p, GLchar *name = %p)", index, size, type, stride, pointer);
...
{
	TRACE("(GLuint index = %d, GLenum pname = 0x%X, GLuint *params = %p)", index, pname, params);
...
{
	TRACE("(GLuint index = %d, GLint x = %d, GLint y = %d, GLint z = %d, GLint w = %d)", index, x, y, z, w);
...
{
	TRACE("(GLuint program = %d, GLint location = %d, GLuint *params = %p)", program, location, params);
...
{
	TRACE("(GLint location = %d, GLsizei count = %d, const GLuint *value = %p)", location, count, value);
...
{
	TRACE("(GLenum buffer = 0x%X, GLint drawbuffer = %d, const GLint *value = %p)", buffer, drawbuffer, value);
	...
	if(context)
	{
		...
		{
		case GL_COLOR:
			if(drawbuffer < 0 || drawbuffer >= es2::MAX_DRAW_BUFFERS)
			{
				return es2::error(GL_INVALID_VALUE);
			}
			else
...
{
	TRACE("(GLenum buffer = 0x%X, GLint drawbuffer = %d, const GLuint *value = %p)", buffer, drawbuffer, value);
...
{
	TRACE("(GLenum buffer = 0x%X, GLint drawbuffer = %d, const GLfloat *value = %p)", buffer, drawbuffer, value);
...
{
	TRACE("(GLenum buffer = 0x%X, GLint drawbuffer = %d, GLfloat depth = %f, GLint stencil = %d)", buffer, drawbuffer, depth, stencil);
...
const GLubyte *GL_APIENTRY GetStringi(GLenum name, GLuint index)
{
	...
	if(context)
	{
		...
		{
			return es2::error(GL_INVALID_VALUE, (GLubyte*)nullptr);
		}
...
{
	TRACE("(GLenum readTarget = 0x%X, GLenum writeTarget = 0x%X,  GLintptr readOffset = %d, GLintptr writeOffset = %d, GLsizeiptr size = %d)", readTarget, writeTarget, readOffset, writeOffset, size);
...
{
	TRACE("(GLuint program = %d, GLsizei uniformCount = %d, const GLchar *const*uniformNames = %p, GLuint *uniformIndices = %p)", program, uniformCount, uniformNames, uniformIndices);
...
{
	TRACE("(GLuint program = %d, GLsizei uniformCount = %d, const GLchar *const*uniformNames = %p, GLenum pname = 0x%X, GLuint *uniformIndices = %p)", program, uniformCount, uniformIndices, pname, uniformIndices);
...
{
	TRACE("(GLuint program = %d, const GLchar *uniformBlockName = %p)", program, uniformBlockName);
	...
	if(context)
	{
		...
		if(!programObject)
		{
			if(context->getShader(program))
			{
				return es2::error(GL_INVALID_OPERATION, GL_INVALID_INDEX);
			}
			else
			{
				return es2::error(GL_INVALID_VALUE, GL_INVALID_INDEX);
			}
		}
...
{
	TRACE("(GLuint program = %d, GLuint uniformBlockIndex = %d, GLenum pname = 0x%X, GLint *params = %p)", program, uniformBlockIndex, pname, params);
...
{
	TRACE("(GLuint program = %d, GLuint uniformBlockIndex = %d, GLsizei bufSize = %d, GLsizei *length = %p, GLchar *uniformBlockName = %p)", program, uniformBlockIndex, bufSize, length, uniformBlockName);
...
{
	TRACE("(GLuint program = %d, GLuint uniformBlockIndex = %d, GLuint uniformBlockBinding = %d)", program, uniformBlockIndex, uniformBlockBinding);

	if(uniformBlockBinding >= es2::MAX_UNIFORM_BUFFER_BINDINGS)
	{
		return es2::error(GL_INVALID_VALUE);
	}
...
GLenum GL_APIENTRY ClientWaitSync(GLsync sync, GLbitfield flags, GLuint64 timeout)
{
	...
	{
		return es2::error(GL_INVALID_VALUE, GL_FALSE);
	}
		...
		if(!(context->getIntegerv(pname, data)))
		{
			...
			else if(nativeType == GL_FLOAT)
			{
				...
				for(unsigned int i = 0; i < numParams; ++i)
				{
					...
					{
						data[i] = (GLint64)(sw::convert_float_fixed(floatParams[i]));
					}
					else
					{
...
{
	TRACE("(GLsync sync = %p, GLenum pname = 0x%X, GLsizei bufSize = %d, GLsizei *length = %p, GLint *values = %p)", sync, pname, bufSize, length, values);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, const GLint *param = %p)", sampler, pname, param);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, GLint param = %d)", sampler, pname, param);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, const GLfloat *param = %p)", sampler, pname, param);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, GLfloat param = %f)", sampler, pname, param);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, GLint *params = %p)", sampler, pname, params);
...
{
	TRACE("(GLuint sampler = %d, GLenum pname = 0x%X, GLfloat *params = %p)", sampler, pname, params);
...
{
	TRACE("(GLuint program = %d, GLsizei bufSize = %d, GLsizei *length = %p, GLenum *binaryFormat = %p, void *binary = %p)", program, bufSize, length, binaryFormat, binary);
...
{
	TRACE("(GLuint program = %d, GLenum binaryFormat = 0x%X, const void *binary = %p, GLsizei length = %d)", program, binaryFormat, binaryFormat, length);
...
{
	TRACE("(GLuint program = %d, GLenum pname = 0x%X, GLint value = %d)", program, pname, value);
...
{
	TRACE("(GLenum target = 0x%X, GLsizei numAttachments = %d, const GLenum *attachments = %p, GLint x = %d, GLint y = %d, GLsizei width = %d, GLsizei height = %d)", target, numAttachments, attachments, x, y, width, height);
		...
		if(framebuffer)
		{
			for(int i = 0; i < numAttachments; i++)
			{
				...
				default:
					...
					{
						if(attachments[i] - GL_COLOR_ATTACHMENT0 >= es2::MAX_DRAW_BUFFERS)
						{
							return es2::error(GL_INVALID_OPERATION);
						}
					}
...
{
	TRACE("(GLenum target = 0x%X, GLsizei numAttachments = %d, const GLenum *attachments = %p)", target, numAttachments, attachments);
...
{
	TRACE("(GLenum target = 0x%X, GLsizei levels = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d)", target, levels, internalformat, width, height);
	...
	bool isCompressed = es2::IsCompressed(internalformat);
	if(!es2::IsSizedInternalFormat(internalformat) && !isCompressed)
	{
		return es2::error(GL_INVALID_ENUM);
	}
...
{
	TRACE("(GLenum target = 0x%X, GLsizei levels = %d, GLenum internalformat = 0x%X, GLsizei width = %d, GLsizei height = %d, GLsizei depth = %d)", target, levels, internalformat, width, height, depth);
	...
	if(!es2::IsSizedInternalFormat(internalformat) && !es2::IsCompressed(internalformat))
	{
		return es2::error(GL_INVALID_ENUM);
	}
...
{
	TRACE("(GLenum target = 0x%X, GLenum internalformat = 0x%X, GLenum pname = 0x%X, GLsizei bufSize = %d, GLint *params = %p)", target, internalformat, pname, bufSize, params);
	...
	if(!es2::IsColorRenderable(internalformat) &&
	   !es2::IsDepthRenderable(internalformat) &&
	   !es2::IsStencilRenderable(internalformat))
	{
		return es2::error(GL_INVALID_ENUM);
	}
	...
	GLint numMultisampleCounts = es2::NUM_MULTISAMPLE_COUNTS;

	// Integer types have no multisampling.
	GLenum type = es2::GetColorComponentType(internalformat);
	...
	case GL_SAMPLES:
		...
		{
			params[i] = es2::multisampleCount[i];
		}
		break;
	default:
		return es2::error(GL_INVALID_ENUM);
