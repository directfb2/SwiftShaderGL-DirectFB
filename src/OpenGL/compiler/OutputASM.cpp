...
GLenum glVariableType(const TType &type)
{
	...
	case EbtFloat:
		...
		else if(type.isVector())
		{
			...
			default:
				UNREACHABLE(type.getNominalSize());
				return GL_NONE;
			}
		}
		else if(type.isMatrix())
		{
			...
			case 2:
				...
				default:
					UNREACHABLE(type.getSecondarySize());
					return GL_NONE;
				}
			case 3:
				switch(type.getSecondarySize())
...
void OutputASM::visitSymbol(TIntermSymbol *symbol)
{
	...
	// so declare them but don't assign a register index yet.
...
bool OutputASM::visitBinary(Visit visit, TIntermBinary *node)
{
	...
	case EOpVectorTimesMatrixAssign:
		assert(visit == PreVisit);
		{
			// The left operand may contain a swizzle serving double-duty as swizzle and writemask,
			// so it's important that we traverse it first.
			// Otherwise we may end up never setting up our left operand correctly.
			left->traverse(this);
			right->traverse(this);
	...
	case EOpAddAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_ADD, result), result, left, left, right);
		}
		break;
	case EOpAdd:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_ADD, result), result, left, right);
		}
		break;
	case EOpSubAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_SUB, result), result, left, left, right);
		}
		break;
	case EOpSub:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_SUB, result), result, left, right);
		}
		break;
	case EOpMulAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_MUL, result), result, left, left, right);
		}
		break;
	case EOpMul:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_MUL, result), result, left, right);
		}
		break;
	case EOpDivAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_DIV, result), result, left, left, right);
		}
		break;
	case EOpDiv:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_DIV, result), result, left, right);
		}
		break;
	case EOpIModAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_IMOD, result), result, left, left, right);
		}
		break;
	case EOpIMod:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_IMOD, result), result, left, right);
		}
		break;
	case EOpBitShiftLeftAssign:
		if(visit == PostVisit)
		{
			emitAssign(sw::Shader::OPCODE_SHL, result, left, left, right);
		}
		break;
	case EOpBitShiftLeft:
		if(visit == PostVisit)
		{
			emitBinary(sw::Shader::OPCODE_SHL, result, left, right);
		}
		break;
	case EOpBitShiftRightAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_ISHR, result), result, left, left, right);
		}
		break;
	case EOpBitShiftRight:
		if(visit == PostVisit)
		{
			emitBinary(getOpcode(sw::Shader::OPCODE_ISHR, result), result, left, right);
		}
		break;
	case EOpBitwiseAndAssign:
		if(visit == PostVisit)
		{
			emitAssign(sw::Shader::OPCODE_AND, result, left, left, right);
		}
		break;
	case EOpBitwiseAnd:
		if(visit == PostVisit)
		{
			emitBinary(sw::Shader::OPCODE_AND, result, left, right);
		}
		break;
	case EOpBitwiseXorAssign:
		if(visit == PostVisit)
		{
			emitAssign(sw::Shader::OPCODE_XOR, result, left, left, right);
		}
		break;
	case EOpBitwiseXor:
		if(visit == PostVisit)
		{
			emitBinary(sw::Shader::OPCODE_XOR, result, left, right);
		}
		break;
	case EOpBitwiseOrAssign:
		if(visit == PostVisit)
		{
			emitAssign(sw::Shader::OPCODE_OR, result, left, left, right);
		}
		break;
	case EOpBitwiseOr:
		if(visit == PostVisit)
		{
			emitBinary(sw::Shader::OPCODE_OR, result, left, right);
		}
		break;
	case EOpEqual:
		if(visit == PostVisit)
	...
	case EOpLessThan:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_LT, result, left, right);
		}
		break;
	case EOpGreaterThan:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_GT, result, left, right);
		}
		break;
	case EOpLessThanEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_LE, result, left, right);
		}
		break;
	case EOpGreaterThanEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_GE, result, left, right);
		}
		break;
	case EOpVectorTimesScalarAssign:
		if(visit == PostVisit)
		{
			emitAssign(getOpcode(sw::Shader::OPCODE_MUL, left), result, left, left, right);
		}
		break;
	case EOpVectorTimesScalar:
		if(visit == PostVisit)
		{
			emit(getOpcode(sw::Shader::OPCODE_MUL, left), result, left, right);
		}
		break;
	...
	case EOpLogicalXor:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_XOR, result, left, right);
		}
		break;
	case EOpLogicalAnd:
		if(trivial(right, 6))
...
bool OutputASM::visitUnary(Visit visit, TIntermUnary *node)
{
	...
	case EOpVectorLogicalNot:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_NOT, result, arg);
		}
		break;
	case EOpLogicalNot:
		if(visit == PostVisit)
	...
	case EOpRadians:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_MUL, result, arg, &rad);
		}
		break;
	case EOpDegrees:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_MUL, result, arg, &deg);
		}
		break;
	case EOpSin:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_SIN, result, arg);
		}
		break;
	case EOpCos:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_COS, result, arg);
		}
		break;
	case EOpTan:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_TAN, result, arg);
		}
		break;
	case EOpAsin:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ASIN, result, arg);
		}
		break;
	case EOpAcos:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ACOS, result, arg);
		}
		break;
	case EOpAtan:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ATAN, result, arg);
		}
		break;
	case EOpSinh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_SINH, result, arg);
		}
		break;
	case EOpCosh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_COSH, result, arg);
		}
		break;
	case EOpTanh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_TANH, result, arg);
		}
		break;
	case EOpAsinh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ASINH, result, arg);
		}
		break;
	case EOpAcosh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ACOSH, result, arg);
		}
		break;
	case EOpAtanh:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ATANH, result, arg);
		}
		break;
	case EOpExp:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_EXP, result, arg);
		}
		break;
	case EOpLog:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_LOG, result, arg);
		}
		break;
	case EOpExp2:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_EXP2, result, arg);
		}
		break;
	case EOpLog2:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_LOG2, result, arg);
		}
		break;
	case EOpSqrt:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_SQRT, result, arg);
		}
		break;
	case EOpInverseSqrt:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_RSQ, result, arg);
		}
		break;
	case EOpAbs:
		if(visit == PostVisit)
		{
			emit(getOpcode(sw::Shader::OPCODE_ABS, result), result, arg);
		}
		break;
	case EOpSign:
		if(visit == PostVisit)
		{
			emit(getOpcode(sw::Shader::OPCODE_SGN, result), result, arg);
		}
		break;
	case EOpFloor:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FLOOR, result, arg);
		}
		break;
	case EOpTrunc:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_TRUNC, result, arg);
		}
		break;
	case EOpRound:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ROUND, result, arg);
		}
		break;
	case EOpRoundEven:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ROUNDEVEN, result, arg);
		}
		break;
	case EOpCeil:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_CEIL, result, arg, result);
		}
		break;
	case EOpFract:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FRC, result, arg);
		}
		break;
	case EOpIsNan:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ISNAN, result, arg);
		}
		break;
	case EOpIsInf:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ISINF, result, arg);
		}
		break;
	case EOpLength:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_LEN(dim(arg)), result, arg);
		}
		break;
	case EOpNormalize:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_NRM(dim(arg)), result, arg);
		}
		break;
	case EOpDFdx:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_DFDX, result, arg);
		}
		break;
	case EOpDFdy:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_DFDY, result, arg);
		}
		break;
	case EOpFwidth:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FWIDTH, result, arg);
		}
		break;
	case EOpAny:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ANY, result, arg);
		}
		break;
	case EOpAll:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ALL, result, arg);
		}
		break;
	case EOpFloatBitsToInt:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FLOATBITSTOINT, result, arg);
		}
		break;
	case EOpFloatBitsToUint:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FLOATBITSTOUINT, result, arg);
		}
		break;
	case EOpIntBitsToFloat:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_INTBITSTOFLOAT, result, arg);
		}
		break;
	case EOpUintBitsToFloat:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_UINTBITSTOFLOAT, result, arg);
		}
		break;
	case EOpPackSnorm2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_PACKSNORM2x16, result, arg);
		}
		break;
	case EOpPackUnorm2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_PACKUNORM2x16, result, arg);
		}
		break;
	case EOpPackHalf2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_PACKHALF2x16, result, arg);
		}
		break;
	case EOpUnpackSnorm2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_UNPACKSNORM2x16, result, arg);
		}
		break;
	case EOpUnpackUnorm2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_UNPACKUNORM2x16, result, arg);
		}
		break;
	case EOpUnpackHalf2x16:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_UNPACKHALF2x16, result, arg);
		}
		break;
	case EOpTranspose:
		if(visit == PostVisit)
...
bool OutputASM::visitAggregate(Visit visit, TIntermAggregate *node)
{
	...
	case EOpLessThan:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_LT, result, arg[0], arg[1]);
		}
		break;
	case EOpGreaterThan:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_GT, result, arg[0], arg[1]);
		}
		break;
	case EOpLessThanEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_LE, result, arg[0], arg[1]);
		}
		break;
	case EOpGreaterThanEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_GE, result, arg[0], arg[1]);
		}
		break;
	case EOpVectorEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_EQ, result, arg[0], arg[1]);
		}
		break;
	case EOpVectorNotEqual:
		if(visit == PostVisit)
		{
			emitCmp(sw::Shader::CONTROL_NE, result, arg[0], arg[1]);
		}
		break;
	case EOpMod:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_MOD, result, arg[0], arg[1]);
		}
		break;
	case EOpModf:
		if(visit == PostVisit)
	...
	case EOpPow:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_POW, result, arg[0], arg[1]);
		}
		break;
	case EOpAtan:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_ATAN2, result, arg[0], arg[1]);
		}
		break;
	case EOpMin:
		if(visit == PostVisit)
		{
			emit(getOpcode(sw::Shader::OPCODE_MIN, result), result, arg[0], arg[1]);
		}
		break;
	case EOpMax:
		if(visit == PostVisit)
	...
	case EOpStep:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_STEP, result, arg[0], arg[1]);
		}
		break;
	case EOpSmoothStep:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_SMOOTH, result, arg[0], arg[1], arg[2]);
		}
		break;
	case EOpDistance:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_DIST(dim(arg[0])), result, arg[0], arg[1]);
		}
		break;
	case EOpDot:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_DP(dim(arg[0])), result, arg[0], arg[1]);
		}
		break;
	case EOpCross:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_CRS, result, arg[0], arg[1]);
		}
		break;
	case EOpFaceForward:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_FORWARD(dim(arg[0])), result, arg[0], arg[1], arg[2]);
		}
		break;
	case EOpReflect:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_REFLECT(dim(arg[0])), result, arg[0], arg[1]);
		}
		break;
	case EOpRefract:
		if(visit == PostVisit)
		{
			emit(sw::Shader::OPCODE_REFRACT(dim(arg[0])), result, arg[0], arg[1], arg[2]);
		}
		break;
	case EOpMul:
		if(visit == PostVisit)
...
void OutputASM::source(sw::Shader::SourceParameter &parameter, TIntermNode *argument, int index)
{
	if(argument)
	{
		...
		else
		{
			...
			{
				mContext.error(arg->getLine(), "Too many temporary registers required to compile shader", pixelShader ? "pixel shader" : "vertex shader");
			}
		}
...
sw::Shader::ParameterType OutputASM::registerType(TIntermTyped *operand)
{
	...
	case EvqConstExpr:           return sw::Shader::PARAMETER_FLOAT4LITERAL;
	...
	case EvqInvariantVaryingIn:  return sw::Shader::PARAMETER_INPUT;
	case EvqInvariantVaryingOut: return sw::Shader::PARAMETER_OUTPUT;
...
unsigned int OutputASM::registerIndex(TIntermTyped *operand)
{
	...
	case EvqConstExpr:           return temporaryRegister(operand);
	...
	default:
		UNREACHABLE(operand->getQualifier());
		return 0;
	}
}
