...
{
       0,    99,    99,   106,   107,   117,   117,   135,   135,   153,
     156,   159,   162,   165,   168,   171,   174,   177,   180,   203,
     222,   225,   228,   252,   276,   279,   282,   285,   297,   300
};
#endif
...
yyreduce:
  ...
  case 4:

    {
        if (!context->isIgnoringErrors())
        {
            ...
            context->diagnostics->report(context->errorSettings.unexpectedIdentifier, context->token->location, context->token->text);
            *(context->valid) = false;
        }
        (yyval) = (yyvsp[0]);
    }
...

    {
        if ((yyvsp[0]) < 0 || (yyvsp[0]) > 31)
        {
            if (!context->isIgnoringErrors())
            {
                ...
                context->diagnostics->report(pp::Diagnostics::PP_UNDEFINED_SHIFT, context->token->location, text.c_str());
                *(context->valid) = false;
            }
            (yyval) = static_cast<YYSTYPE>(0);
        }
...

    {
        if ((yyvsp[0]) == 0)
        {
            if (!context->isIgnoringErrors())
            {
                ...
                stream << (yyvsp[-2]) << " % " << (yyvsp[0]);
                std::string text = stream.str();
                context->diagnostics->report(pp::Diagnostics::PP_DIVISION_BY_ZERO, context->token->location, text.c_str());
                *(context->valid) = false;
            }
            (yyval) = static_cast<YYSTYPE>(0);
        }
  ...
  case 27:
    ...
    {
        // Check for negation of minimum representable integer to prevent
        // undefined signed int overflow.
...
int yylex(YYSTYPE *lvalp, Context *context)
{
    ...
    switch (token->type) {
      case pp::Token::CONST_INT: {
        ...
        {
            context->diagnostics->report(pp::Diagnostics::PP_INTEGER_OVERFLOW, token->location, token->text);
            *(context->valid) = false;
        }
...
{
    context->diagnostics->report(pp::Diagnostics::PP_INVALID_EXPRESSION, context->token->location, reason);
}
