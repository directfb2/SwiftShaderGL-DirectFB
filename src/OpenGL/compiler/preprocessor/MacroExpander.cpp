...
MacroExpander::ScopedMacroReenabler::~ScopedMacroReenabler()
{
	...
	{
		// Copying the string here by using substr is a check for use-after-free.
		// It detects use-after-free more reliably than just toggling the disabled flag.
...
bool MacroExpander::expandMacro(const Macro &macro, const Token &identifier, std::vector<Token> *replacements)
{
	replacements->clear();
                                     ...
                                     SourceLocation *closingParenthesisLocation)
{
	...
	// Defer reenabling macros until args collection is finished to avoid the
	// possibility of infinite recursion.
	// Otherwise infinite recursion might happen when expanding the args after
	...
	while(openParens != 0)
	{
		...
		{
			mDiagnostics->report(Diagnostics::PP_MACRO_UNTERMINATED_INVOCATION, identifier.location, identifier.text);
	...
	// This step expands each argument individually before they are inserted
	// into the macro body.
	size_t numTokens = 0;
	for(auto &arg : *args)
	{
		...
		{
			mDiagnostics->report(Diagnostics::PP_MACRO_INVOCATION_CHAIN_TOO_DEEP, token.location, token.text);
			return false;
		}
                                       ...
                                       std::vector<Token> *replacements)
{
	...
	{
		if(!replacements->empty() && replacements->size() + mTotalTokensInContexts > kMaxContextTokens)
		{
		...
		Macro::Parameters::const_iterator iter = std::find(macro.parameters.begin(), macro.parameters.end(), repl.text);
		...
		// The replacement token inherits padding properties from macro replacement token.
