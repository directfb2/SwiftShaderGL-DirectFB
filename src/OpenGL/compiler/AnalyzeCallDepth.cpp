...
static TIntermSequence::iterator traverseCaseBody(AnalyzeCallDepth* analysis, TIntermSequence::iterator& start, const TIntermSequence::iterator& end)
{
	TIntermSequence::iterator current = start;
	for(++current; current != end; ++current)
	{
		(*current)->traverse(analysis);
		if((*current)->getAsBranchNode())
		{
			break;
		}
	}
