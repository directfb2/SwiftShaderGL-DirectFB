...
TIntermTyped* TIntermediate::addBinaryMath(TOperator op, TIntermTyped* left, TIntermTyped* right, const TSourceLoc &line)
{
	...
	// Make a new node and promote it to the right type.
...
// a true path, and a false path. The two paths are in the nodePair.
//
// Returns the selection node created.
//
TIntermNode* TIntermediate::addSelection(TIntermTyped* cond, TIntermNodePair nodePair, const TSourceLoc &line)
{
	// For compile time constant selections, prune code and test now.
...
TIntermTyped* TIntermediate::addSelection(TIntermTyped* cond, TIntermTyped* trueBlock, TIntermTyped* falseBlock, const TSourceLoc &line)
{
	...
	// If all the operands are constant, then fold it.
...
ConstantUnion* CreateInverse(TIntermConstantUnion* node, ConstantUnion* unionArray)
{
	...
	if(determinant != 0.0f)
	{
		...
		case 2:
			{
				float m00 = unionArray[0].getFConst();
				float m01 = unionArray[1].getFConst();
				float m10 = unionArray[2].getFConst();
		...
		case 3:
			{
				float m00 = unionArray[0].getFConst();
				float m01 = unionArray[1].getFConst();
				float m02 = unionArray[2].getFConst();
				float m10 = unionArray[3].getFConst();
				...
				tempConstArray[0].setFConst(invDet * determinant2(m11, m12, m21, m22));
				tempConstArray[1].setFConst(invDet * determinant2(m12, m10, m22, m20));
				tempConstArray[2].setFConst(invDet * determinant2(m10, m11, m20, m21));
				tempConstArray[3].setFConst(invDet * determinant2(m21, m22, m01, m02));
				tempConstArray[4].setFConst(invDet * determinant2(m00, m02, m20, m22));
				tempConstArray[5].setFConst(invDet * determinant2(m20, m21, m00, m01));
				tempConstArray[6].setFConst(invDet * determinant2(m01, m02, m11, m12));
				tempConstArray[7].setFConst(invDet * determinant2(m10, m12, m00, m02));
				tempConstArray[8].setFConst(invDet * determinant2(m00, m01, m10, m11));
			}
			break;
		case 4:
			{
				...
				float m01 = unionArray[1].getFConst();
				float m02 = unionArray[2].getFConst();
				float m03 = unionArray[3].getFConst();
				float m10 = unionArray[4].getFConst();
				...
				tempConstArray[ 0].setFConst( invDet * determinant3(m11, m12, m13, m21, m22, m23, m31, m32, m33));
				tempConstArray[ 1].setFConst(-invDet * determinant3(m10, m12, m13, m20, m22, m23, m30, m32, m33));
				tempConstArray[ 2].setFConst( invDet * determinant3(m10, m11, m13, m20, m21, m23, m30, m31, m33));
				tempConstArray[ 3].setFConst(-invDet * determinant3(m10, m11, m12, m20, m21, m22, m30, m31, m32));
				tempConstArray[ 4].setFConst( invDet * determinant3(m01, m02, m03, m21, m22, m23, m31, m32, m33));
				tempConstArray[ 5].setFConst(-invDet * determinant3(m00, m02, m03, m20, m22, m23, m30, m32, m33));
				tempConstArray[ 6].setFConst( invDet * determinant3(m00, m01, m03, m20, m21, m23, m30, m31, m33));
				tempConstArray[ 7].setFConst(-invDet * determinant3(m00, m01, m02, m20, m21, m22, m30, m31, m32));
				tempConstArray[ 8].setFConst( invDet * determinant3(m01, m02, m03, m11, m12, m13, m31, m32, m33));
				tempConstArray[ 9].setFConst(-invDet * determinant3(m00, m02, m03, m10, m12, m13, m30, m32, m33));
				tempConstArray[10].setFConst( invDet * determinant3(m00, m01, m03, m10, m11, m13, m30, m31, m33));
				tempConstArray[11].setFConst(-invDet * determinant3(m00, m01, m02, m10, m11, m12, m30, m31, m32));
				tempConstArray[12].setFConst( invDet * determinant3(m01, m02, m03, m11, m12, m13, m21, m22, m23));
				tempConstArray[13].setFConst(-invDet * determinant3(m00, m02, m03, m10, m12, m13, m20, m22, m23));
				tempConstArray[14].setFConst( invDet * determinant3(m00, m01, m03, m10, m11, m13, m20, m21, m23));
				tempConstArray[15].setFConst(-invDet * determinant3(m00, m01, m02, m10, m11, m12, m20, m21, m22));
			}
			break;
		default:
			UNREACHABLE(size);
