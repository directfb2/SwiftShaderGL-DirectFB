...
bool ValidateSwitch::validate(TBasicType switchType, TParseContext *context, TIntermAggregate *statementList, const TSourceLoc &loc)
{
...
bool ValidateSwitch::visitCase(Visit, TIntermCase *node)
{
	...
	else
	{
		...
		{
			mContext->error(condition->getLine(), "case label type does not match switch init-expression type", nodeStr);
		...
		// Other types are possible only in error cases, where the error has already
		// been generated when parsing the case statement.
	}
...
bool ValidateSwitch::validateInternal(const TSourceLoc &loc)
{
	...
	{
		mContext->error(loc, "no statement between the last label and the end of the switch statement", "switch");
	}

	return !mStatementBeforeCase   &&
