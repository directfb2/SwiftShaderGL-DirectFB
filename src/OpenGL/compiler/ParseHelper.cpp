...
//
void TParseContext::error(const TSourceLoc& loc, const char* reason, const char* token, const char* extraInfo)
{
...
void TParseContext::warning(const TSourceLoc& loc, const char* reason, const char* token, const char* extraInfo)
{
	pp::SourceLocation srcLoc(loc.first_file, loc.first_line);
	mDiagnostics.writeInfo(pp::Diagnostics::PP_WARNING, srcLoc, reason, token, extraInfo);
}

void TParseContext::info(const TSourceLoc& loc, const char* reason, const char* token, const char* extraInfo)
{
	pp::SourceLocation srcLoc(loc.first_file, loc.first_line);
	mDiagnostics.writeInfo(pp::Diagnostics::PP_INFO, srcLoc, reason, token, extraInfo);
}
...
{
	std::stringstream extraInfoStream;
	extraInfoStream << "no operation '" << op << "' exists that takes an operand of type " << operand << " (or there is no acceptable conversion)";
	std::string extraInfo = extraInfoStream.str();
...
{
	std::stringstream extraInfoStream;
	extraInfoStream << "no operation '" << op << "' exists that takes a left-hand operand of type '" << left << "' and a right operand of type '" << right << "' (or there is no acceptable conversion)";
	std::string extraInfo = extraInfoStream.str();
...
bool TParseContext::precisionErrorCheck(const TSourceLoc &line, TPrecision precision, TBasicType type)
{
	...
	case EbtFloat:
		...
		{
			error( line, "No precision specified (float)", "" );
			return true;
		}
		break;
	case EbtInt:
...
bool TParseContext::lValueErrorCheck(const TSourceLoc &line, const char* op, TIntermTyped* node)
{
	...
	default:
		// Type in which it is not possible to write.
...
bool TParseContext::constructorErrorCheck(const TSourceLoc &line, TIntermNode* node, TFunction& function, TOperator op, TType* type)
{
	*type = function.getReturnType();
	...
	// Note: It's okay to have too many components available, but not okay to have
	// unused arguments. 'full' will go to true when enough args have been seen.
	// If we loop again, there's an extra argument, so 'overfull' will become true.
...
//
// This function checks to see if a void variable has been declared and raise
// an error message for such a case.
//
// Returns true if there was an error.
//
...
//
// This function checks to see if the node (for the expression) contains
// a scalar boolean expression or not.
//
// Returns true if there was an error.
//
...
//
// These checks are common for all declarations starting a declarator list,
// and declarators that follow an empty declaration.
//
bool TParseContext::singleDeclarationErrorCheck(const TPublicType &publicType, const TSourceLoc &identifierLocation)
{
	...
	case EvqFragmentOut:
		...
		{
			error(identifierLocation, "cannot be used with a structure", getQualifierString(publicType.qualifier));
			return true;
		}
...
bool TParseContext::arrayTypeErrorCheck(const TSourceLoc &line, TPublicType type)
{
	...
	// In ESSL1.00 shaders, structs cannot be varying.
	// In ESSL3.00 shaders, struct inputs/outputs are allowed but not arrays of structs.
...
bool TParseContext::arraySetMaxSize(TIntermSymbol *node, TType* type, int size, bool updateFlag, const TSourceLoc &line)
{
	...
	// Special casing to test index value of gl_FragData.
	...
	// Don't update the maxArraySize when this flag is not set.
	if(!updateFlag)
		return false;
...
//
bool TParseContext::declareVariable(const TSourceLoc &line, const TString &identifier, const TType &type, TVariable **variable)
{
...
bool TParseContext::functionCallLValueErrorCheck(const TFunction *fnCandidate, TIntermAggregate *aggregate)
{
	for(size_t i = 0; i < fnCandidate->getParamCount(); ++i)
	{
		...
		if(qual == EvqOut || qual == EvqInOut)
		{
			...
			{
				error(node->getLine(), "Constant value cannot be passed for 'out' or 'inout' parameters.", "Error");
				recover();
				return true;
			}
		}
...
const TVariable *TParseContext::getNamedVariable(const TSourceLoc &location, const TString *name, const TSymbol *symbol)
{
	...
	else
	{
		...
		// It's only an error to write to both FragData and FragColor.
		// For simplicity, return an error if they are both referenced,
...
//
bool TParseContext::executeInitializer(const TSourceLoc& line, const TString& identifier, const TPublicType& pType, TIntermTyped *initializer, TIntermNode **intermNode)
{
	...
	if(type.isUnsizedArray())
	{
		...
		{
			// Having a non-array initializer for an unsized array will result
			// in an error later, so we don't generate an error message here.
...
void TParseContext::checkInputOutputTypeIsValidES3(const TQualifier qualifier, const TPublicType &type, const TSourceLoc &qualifierLocation)
{
	// An input/output variable can never be bool or a sampler.
	// Samplers are checked elsewhere.
	...
	// Vertex shader outputs / fragment shader inputs have a different, slightly
	// more lenient set of restrictions.
...
TIntermAggregate *TParseContext::parseSingleDeclaration(TPublicType &publicType, const TSourceLoc &identifierOrTypeLocation, const TString &identifier)
{
	...
	if(emptyDeclaration)
	{
		...
		{
			// Array declaration which leaves the size unspecified is an error.
...
TIntermAggregate *TParseContext::parseSingleArrayDeclaration(TPublicType &publicType, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &indexLocation, TIntermTyped *indexExpression)
{
	...
	// Make the type an array even if size check failed. This ensures useless
	// error messages regarding the variable's non-arrayness won't follow.
...
TIntermAggregate *TParseContext::parseSingleInitDeclaration(const TPublicType &publicType, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &initLocation, TIntermTyped *initializer)
{
	mDeferredSingleDeclarationErrorCheck = false;
...
TIntermAggregate *TParseContext::parseSingleArrayInitDeclaration(TPublicType &publicType, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &indexLocation, TIntermTyped *indexExpression, const TSourceLoc &initLocation, TIntermTyped *initializer)
{
	mDeferredSingleDeclarationErrorCheck = false;
	...
	int size = 0;
	// If indexExpression is nullptr, then the array will eventually get its size
	// implicitly from the initializer.
...
TIntermAggregate *TParseContext::parseInvariantDeclaration(const TSourceLoc &invariantLoc, const TSourceLoc &identifierLoc, const TString *identifier, const TSymbol *symbol)
{
...
TIntermAggregate *TParseContext::parseDeclarator(TPublicType &publicType, TIntermAggregate *aggregateDeclaration, const TSourceLoc &identifierLocation, const TString &identifier)
{
	// If the declaration starting this declarator list was empty (example: int,),
...
TIntermAggregate *TParseContext::parseArrayDeclarator(TPublicType &publicType, TIntermAggregate *aggregateDeclaration, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &arrayLocation, TIntermTyped *indexExpression)
{
	// If the declaration starting this declarator list was empty (example: int,),
	// some checks were not performed.
	if(mDeferredSingleDeclarationErrorCheck)
...
TIntermAggregate *TParseContext::parseInitDeclarator(const TPublicType &publicType, TIntermAggregate *aggregateDeclaration, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &initLocation, TIntermTyped *initializer)
{
	// If the declaration starting this declarator list was empty (example: int,),
	// some checks were not performed.
	if(mDeferredSingleDeclarationErrorCheck)
...
TIntermAggregate *TParseContext::parseArrayInitDeclarator(const TPublicType &publicType, TIntermAggregate *aggregateDeclaration, const TSourceLoc &identifierLocation, const TString &identifier, const TSourceLoc &indexLocation, TIntermTyped *indexExpression, const TSourceLoc &initLocation, TIntermTyped *initializer)
{
	// If the declaration starting this declarator list was empty (example: int,),
	// some checks were not performed.
	if(mDeferredSingleDeclarationErrorCheck)
...
{
	// Note: symbolTableFunction could be the same as function if this is the
	// first declaration. Either way the instance in the symbol table is used
	// to track whether the function is declared multiple times.
...
TIntermAggregate *TParseContext::addFunctionDefinition(const TFunction &function, TIntermAggregate *functionPrototype, TIntermAggregate *functionBody, const TSourceLoc &location)
{
	...
	// Store the pragma information for debug and optimize.
	// This information can be queried from the parse tree.
...
void TParseContext::parseFunctionPrototype(const TSourceLoc &location, TFunction *function, TIntermAggregate **aggregateOut)
{
	...
	// Note: 'prevDec' could be 'function' if this is the first time we've seen
	// function as it would have just been put in the symbol table.
	// Otherwise, we're looking up an earlier occurance.
	...
	// Raise error message if main function takes any parameters or return anything
	// other than void.
	...
	for(size_t i = 0; i < function->getParamCount(); i++)
	{
		...
		if(param.name != 0)
		{
			...
			TIntermSymbol *symbol = intermediate.addSymbol(variable->getUniqueId(), variable->getName(), variable->getType(), location);
...
{
	// We don't know at this point whether this is a function definition or
	// a prototype. The definition production code will check for redefinitions.
	// In the case of ESSL 1.00 the prototype production code will also check
	// for redeclarations.
	// Return types and parameter qualifiers must match in all redeclarations,
	// so those are checked here.
	TFunction *prevDec = static_cast<TFunction *>(symbolTable.find(function->getMangledName(), getShaderVersion()));
	...
	else if(prevDec)
	{
		...
		{
			error(location, "overloaded functions must have the same return type", function->getReturnType().getBasicString());
			recover();
		}
		for(size_t i = 0; i < prevDec->getParamCount(); ++i)
		{
			...
			{
				error(location, "overloaded functions must have the same parameter qualifiers", function->getParam(i).type->getQualifierString());
				recover();
			}
		}
	}
	...
	// If this is a redeclaration, it could also be a definition, in which case,
	// we want to use the variable names from this one, and not the one that's
	// being redeclared. So, pass back up this declaration, not the one in the
	// symbol table.
	return function;
}
...
//
// This function is used to test for the correctness of the parameters passed to
// various constructor functions and also convert them to the right datatype if
// it is allowed and required.
//
// Returns 0 for an error or the constructed node (aggregate or typed).
//
...
// accessed from contant vector. If only one component of vector is accessed
// (v.x or v[0] where v is a contant vector), then a contant node is returned,
// else an aggregate node is returned (for v.xy).
// The input to this function could either be the symbol node or it could be the
// intermediate tree representation of accessing fields in a constant structure
// or column of a constant matrix.
//
...
//
// This function returns the column being accessed from a constant matrix.
// The values are retrieved from the symbol table and parse-tree is built
// for a vector (each column of a matrix is a vector).
// The input to the function could either be a symbol node (m[0] where m is
// a constant matrix) that represents a constant matrix or it could be the
// tree representation of the constant matrix (s.m1[0] where s is a constant
// structure)
//
...
// The input to the function could either be a symbol node (a[0] where a is
// a constant array) that represents a constant array or it could be the
// tree representation of the constant array (s.a1[0] where s is a constant
// structure)
//
...
//
// This function returns the value of a particular field inside a constant
// structure from the symbol table. If there is an embedded/nested struct,
// it appropriately calls addConstStructNested or addConstStructFromAggr
// function and returns the parse-tree with the values of the embedded/nested
// struct.
//
...
//
TIntermAggregate* TParseContext::addInterfaceBlock(const TPublicType& typeQualifier, const TSourceLoc& nameLine, const TString& blockName, TFieldList* fieldList, const TString* instanceName, const TSourceLoc& instanceLine, TIntermTyped* arrayIndex, const TSourceLoc& arrayIndexLine)
{
	...
	for(const auto &field : *fieldList)
	{
		...
		// Recursively propagate the matrix packing setting down to all
...
TIntermTyped *TParseContext::addIndexExpression(TIntermTyped *baseExpression, const TSourceLoc &location, TIntermTyped *indexExpression)
{
	...
	if(indexExpression->getQualifier() == EvqConstExpr && indexConstantUnion)
	{
		...
		if(baseExpression->getType().getQualifier() == EvqConstExpr && baseExpression->getAsConstantUnion())
		{
	...
	else if(baseExpression->isArray())
	{
		...
		{
			indexedExpression->setType(TType(baseExpression->getBasicType(), baseExpression->getPrecision(), EvqTemporary, static_cast<unsigned char>(baseExpression->getNominalSize()), static_cast<unsigned char>(baseExpression->getSecondarySize())));
		}
	...
	else if(baseExpression->isMatrix())
	{
		...
		indexedExpression->setType(TType(baseExpression->getBasicType(), baseExpression->getPrecision(), qualifier, static_cast<unsigned char>(baseExpression->getSecondarySize())));
	}
	else if(baseExpression->isVector())
	{
		TQualifier qualifier = baseExpression->getType().getQualifier() == EvqConstExpr ? EvqConstExpr : EvqTemporary;
...
TIntermTyped *TParseContext::addFieldSelectionExpression(TIntermTyped *baseExpression, const TSourceLoc &dotLocation, const TString &fieldString, const TSourceLoc &fieldLocation)
{
	...
	if(baseExpression->isVector())
	{
		...
		else
		{
			...
			indexedExpression->setType(TType(baseExpression->getBasicType(), baseExpression->getPrecision(), baseExpression->getQualifier() == EvqConstExpr ? EvqConstExpr : EvqTemporary, (unsigned char)vectorString.size()));
		}
	}
	...
	else
	{
		if(mShaderVersion < 300)
		{
			error(dotLocation, " field selection requires structure or vector on left hand side", fieldString.c_str());
		}
		else
		{
			error(dotLocation, " field selection requires structure, vector, or interface block on left hand side", fieldString.c_str());
		}
		recover();
		indexedExpression = baseExpression;
	}
...
TLayoutQualifier TParseContext::parseLayoutQualifier(const TString &qualifierType, const TSourceLoc& qualifierTypeLine, int intValue, const TSourceLoc& intValueLine)
{
	...
	qualifier.location = -1; // -1 isn't a valid location, it means the value isn't set
	qualifier.matrixPacking = EmpUnspecified;
	qualifier.blockStorage = EbsUnspecified;
...
TPublicType TParseContext::joinInterpolationQualifiers(const TSourceLoc &interpolationLoc, TQualifier interpolationQualifier, const TSourceLoc &storageLoc, TQualifier storageQualifier)
{
...
TPublicType TParseContext::addStructure(const TSourceLoc &structLine, const TSourceLoc &nameLine, const TString *structName, TFieldList *fieldList)
{
	...
	// Store a bool in the struct if we're at global scope.
...
bool TParseContext::binaryOpCommonCheck(TOperator op, TIntermTyped *left, TIntermTyped *right, const TSourceLoc &loc)
{
	...
	case EOpBitwiseOrAssign:
		...
		{
			// If the nominal sizes of operands do not match, one of them must
			// be a scalar.
			if(!left->isScalar() && !right->isScalar())
				return false;

			// In the case of compound assignment other than multiply-assign,
			// the right side needs to be a scalar.
			// Otherwise a vector/matrix would be assigned to a scalar.
			// A scalar can't be shifted by a vector either.
...
TIntermTyped *TParseContext::addBinaryMathInternal(TOperator op, TIntermTyped *left, TIntermTyped *right, const TSourceLoc &loc)
{
	if(!binaryOpCommonCheck(op, left, right, loc))
	{
		return nullptr;
...
TIntermTyped *TParseContext::addFunctionCallOrMethod(TFunction *fnCall, TIntermNode *paramNode, TIntermNode *thisNode, const TSourceLoc &loc, bool *fatalError)
{
	*fatalError = false;
	...
	{
		// This should be a constructor, don't go through the symbol table.
	...
	{
		// Not a constructor, find function in the symbol table.
		...
		if(fnCandidate)
		{
			...
			else
			{
				...
				// The given function is a builtIn function or a userDefined function
				if(!builtIn)
...
//
int PaParseStrings(int count, const char* const string[], const int length[], TParseContext* context)
{
	if((count == 0) || !string)
		return 1;
