...
public:
	...
	void setMatrixPackingIfUnspecified(TLayoutMatrixPacking matrixPacking)
	{
		if(isStruct())
		{
			...
			structure->setMatrixPackingIfUnspecified((layoutQualifier.matrixPacking == EmpUnspecified) ? matrixPacking : layoutQualifier.matrixPacking);
		}
	...
	bool isRegister() const { return !isMatrix() && !structure && !array && !isInterfaceBlock(); }
	...
	//     vec4 field2;
	//   };
	// For type "nesting2", this method would return 2, i.e. the number
