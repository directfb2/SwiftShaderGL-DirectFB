...
namespace {
	...
	// createGDBRegistrationListener() returns a pointer to a singleton.
	auto jitEventListener = llvm::JITEventListener::createGDBRegistrationListener();
...
DebugInfo::DebugInfo(llvm::IRBuilder<> *builder, llvm::LLVMContext *context, llvm::Module *module, llvm::Function *function) :
	builder(builder),
	context(context),
	module(module),
	function(function)
{
	...
	diCU = diBuilder->createCompileUnit(llvm::dwarf::DW_LANG_C, diBuilder->createFile(fileAndDir.first, fileAndDir.second), "Reactor", 0, "", 0);
	...
	auto sp = diBuilder->createFunction(file,                                // scope
	...
	diRootLocation = llvm::DILocation::get(*context, location.line, 0, sp);
...
void DebugInfo::syncScope(Backtrace const &backtrace)
{
	auto shrink = [this](size_t newsize) {
		while(diScope.size() > newsize)
		{
			...
			LOG("- STACK(%d): di: %p, location: %s:%d",
			    int(diScope.size() - 1), scope.di, scope.location.function.file.c_str(), int(scope.location.line));
	...
	for(size_t i = 0; i < diScope.size(); i++)
	{
		...
		{
			LOG("  STACK(%d): Changed function %s -> %s",
			    int(i), oldLocation.function.name.c_str(), newLocation.function.name.c_str());
			shrink(i);
			break;
		}
		...
		if(oldLocation.line > newLocation.line)
		{
			...
			LOG("  STACK(%d): Jumped backwards %d -> %d. di: %p -> %p",
			    int(i), oldLocation.line, newLocation.line, scope.di, di);
			emitPending(scope, builder);
	...
	while(backtrace.size() > diScope.size())
	{
		...
		auto func = diBuilder->createFunction(file,                                // scope
		                                      name,                                // function name
		                                      "",                                  // linkage
		                                      file,                                // file
		                                      location.line,                       // line
		...
		LOG("+ STACK(%d): di: %p, location: %s:%d",
		    int(i), di, location.function.file.c_str(), int(location.line));
	}
}
...
llvm::DILocation *DebugInfo::getLocation(const Backtrace &backtrace, size_t i)
{
	...
	return llvm::DILocation::get(*context, backtrace[i].line, 0, diScope[i].di, i > 0 ? getLocation(backtrace, i - 1) : diRootLocation);
}
...
void DebugInfo::emitPending(Scope &scope, IRBuilder *builder)
{
	...
	LOG("  EMIT(%s): di: %p, location: %s:%d, isAlloca: %s",
	    pending.name.c_str(), scope.di, pending.location.function.file.c_str(), pending.location.line, isAlloca ? "true" : "false");
	...
	{
		builder->SetCurrentDebugLocation(llvm::DILocation::get(*context, pending.diLocation->getLine() + 1, 0, pending.diLocation->getScope(), pending.diLocation->getInlinedAt()));
		Nop();
	}
...
void DebugInfo::registerBasicTypes()
{
	...
	diTypes.emplace(T(Bool::type()), diBuilder->createBasicType("Bool", sizeof(bool), llvm::dwarf::DW_ATE_boolean));
	diTypes.emplace(T(Byte::type()), diBuilder->createBasicType("Byte", 8, llvm::dwarf::DW_ATE_unsigned_char));
	diTypes.emplace(T(SByte::type()), diBuilder->createBasicType("SByte", 8, llvm::dwarf::DW_ATE_signed_char));
	diTypes.emplace(T(Short::type()), diBuilder->createBasicType("Short", 16, llvm::dwarf::DW_ATE_signed));
	diTypes.emplace(T(UShort::type()), diBuilder->createBasicType("UShort", 16, llvm::dwarf::DW_ATE_unsigned));
	diTypes.emplace(T(Int::type()), diBuilder->createBasicType("Int", 32, llvm::dwarf::DW_ATE_signed));
	diTypes.emplace(T(UInt::type()), diBuilder->createBasicType("UInt", 32, llvm::dwarf::DW_ATE_unsigned));
	diTypes.emplace(T(Long::type()), diBuilder->createBasicType("Long", 64, llvm::dwarf::DW_ATE_signed));
	diTypes.emplace(T(Float::type()), diBuilder->createBasicType("Float", 32, llvm::dwarf::DW_ATE_float));
...
Backtrace DebugInfo::getCallerBacktrace(size_t limit) const
{
	// Specify namespace is required here.
...
llvm::DIType *DebugInfo::getOrCreateType(llvm::Type *type)
{
	...
	if(type->isPointerTy())
	{
		auto dbgTy = diBuilder->createPointerType(getOrCreateType(type->getPointerElementType()), sizeof(void*) * 8, alignof(void*) * 8);
...
{
	static std::regex reLocalDecl("^"                                             // line start
