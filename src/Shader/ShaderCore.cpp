...
Float4 logarithm2(RValue<Float4> x, bool absolute, bool pp)
{
	...
	x1 = (x1 - Float4(1.4960938f)) * Float4(256.0f);
...
{
	return exponential2(Float4(1.44269504f) * x, pp);
}

Float4 logarithm(RValue<Float4> x, bool absolute, bool pp)
{
	return Float4(6.93147181e-1f) * logarithm2(x, absolute, pp);
}
...
Float4 arcsin(RValue<Float4> x, bool pp)
{
		...
		return As<Float4>(As<Int4>(half_pi - Sqrt(Float4(1.0f) - absx) * (a0 + absx * (a1 + absx * (a2 + absx * a3)))) ^ (As<Int4>(x) & Int4(0x80000000)));
}
...
static Float4 arctan_01(Float4 x, bool pp)
{
	if(pp)
	{
...
Float4 arctan(RValue<Float4> x, bool pp)
{
	...
	Float4 y = As<Float4>((O & As<Int4>(Float4(1.0f) / absx)) | (~O & As<Int4>(absx)));

	const Float4 half_pi(1.57079632f);
	Float4 theta = arctan_01(y, pp);

	return As<Float4>(((O & As<Int4>(half_pi - theta)) | (~O & As<Int4>(theta))) ^ (As<Int4>(x) & Int4(0x80000000)));
}
...
Float4 arctan(RValue<Float4> y, RValue<Float4> x, bool pp)
{
	...
	Float4 x1 = As<Float4>((Q & As<Int4>(y0)) | (~Q & As<Int4>(x0)));
	Float4 y1 = As<Float4>((Q & As<Int4>(-x0)) | (~Q & As<Int4>(y0)));

	// Mirror to first octant when in second octant
	Int4 O = CmpNLT(y1, x1);
	Float4 x2 = As<Float4>((O & As<Int4>(y1)) | (~O & As<Int4>(x1)));
	Float4 y2 = As<Float4>((O & As<Int4>(x1)) | (~O & As<Int4>(y1)));

	// Approximation of atan in [0..1]
	Int4 zero_x = CmpEQ(x2, Float4(0.0f));
	Int4 inf_y = IsInf(y2);
	Float4 atan2_theta = arctan_01(y2 / x2, pp);
	theta += As<Float4>((~zero_x & ~inf_y & ((O & As<Int4>(half_pi - atan2_theta)) | (~O & (As<Int4>(atan2_theta))))) | (inf_y & As<Int4>(quarter_pi)));

	// Recover loss of precision for tiny theta angles
	Int4 precision_loss = S & Q & O & ~inf_y;
	return As<Float4>((precision_loss & As<Int4>(-atan2_theta)) | (~precision_loss & As<Int4>(theta)));
}
...
void ShaderCore::floatToHalfBits(Float4& dst, const Float4& floatBits, bool storeInUpperBits)
{
	...
	// This version doesn't round to the nearest even in case of a tie as defined
	// by IEEE 754, it rounds to +inf instead of the nearest even.
...
void ShaderCore::logp(Vector4f &dst, const Vector4f &src, unsigned short shaderModel)
{
	if(shaderModel < 0x0200)
	{
		Float4 tmp0 = Abs(src.x);
		Float4 tmp1 = tmp0;
		Int4 r = As<Int4>(As<UInt4>(tmp0) >> 23) - Int4(127);
