...
void VertexProgram::program(UInt &index)
{
	...
	for(size_t i = 0; i < shader->getLength(); i++)
	{
		...
		if(opcode == Shader::OPCODE_DCL ||
		   opcode == Shader::OPCODE_DEF ||
		   opcode == Shader::OPCODE_DEFI ||
		   opcode == Shader::OPCODE_DEFB)
		{
			continue;
		}
		...
		if(dst.type != Shader::PARAMETER_VOID &&
		   dst.type != Shader::PARAMETER_LABEL &&
		   opcode != Shader::OPCODE_NOP)
		{
			if(dst.saturate)
			{
			...
			if(instruction->isPredicated())
			{
				...
				case Shader::PARAMETER_TEXCRDOUT:
					...
					else if(dst.rel.type == Shader::PARAMETER_VOID)
					{
...
Vector4f VertexProgram::fetchRegister(const Src &src, unsigned int offset)
{
	...
	case Shader::PARAMETER_ADDR:
		reg = a0;
		break;
	case Shader::PARAMETER_CONSTBOOL:
		return r[0]; // Dummy
	case Shader::PARAMETER_CONSTINT:
		return r[0]; // Dummy
	case Shader::PARAMETER_LOOP:
		return r[0]; // Dummy
	case Shader::PARAMETER_PREDICATE:
		return r[0]; // Dummy
...
Vector4f VertexProgram::readConstant(const Src &src, unsigned int offset)
{
	...
	else
	{
		...
		{
		case Shader::PARAMETER_ADDR:
			a = a0[component]; break;
		case Shader::PARAMETER_TEMP:
			a = r[src.rel.index][component];
			break;
		case Shader::PARAMETER_INPUT:
			a = v[src.rel.index][component];
			break;
		case Shader::PARAMETER_OUTPUT:
			a = o[src.rel.index][component];
			break;
		case Shader::PARAMETER_CONST:
			a = *Pointer<Float>(uniformAddress(src.bufferIndex, src.rel.index) + component * sizeof(float));
			break;
		case Shader::PARAMETER_MISCTYPE:
			switch(src.rel.index)
			{
			case Shader::InstanceIDIndex:
				a = As<Float4>(Int4(instanceID));
				break;
			case Shader::VertexIDIndex:
		...
		{
			index = Min(As<UInt4>(index), UInt4(VERTEX_UNIFORM_VECTORS)); // Clamp to constant register range
		}
...
Int4 VertexProgram::dynamicAddress(const Shader::Relative &rel)
{
	...
	case Shader::PARAMETER_ADDR:
		...
		break;
	case Shader::PARAMETER_TEMP:
		a = r[rel.index][component];
		break;
	case Shader::PARAMETER_INPUT:
		a = v[rel.index][component];
		break;
	case Shader::PARAMETER_OUTPUT:
		a = o[rel.index][component];
		break;
	case Shader::PARAMETER_MISCTYPE:
		switch(rel.index)
		{
		case Shader::InstanceIDIndex:
			a = As<Float>(instanceID);
			break;
		case Shader::VertexIDIndex:
			a = As<Float4>(vertexID);
			break;
...
void VertexProgram::M3X2(Vector4f &dst, Vector4f &src0, const Src &src1)
{
...
void VertexProgram::M3X3(Vector4f &dst, Vector4f &src0, const Src &src1)
{
	Vector4f row0 = fetchRegister(src1, 0);
	Vector4f row1 = fetchRegister(src1, 1);
...
void VertexProgram::M3X4(Vector4f &dst, Vector4f &src0, const Src &src1)
{
	Vector4f row0 = fetchRegister(src1, 0);
	Vector4f row1 = fetchRegister(src1, 1);
	Vector4f row2 = fetchRegister(src1, 2);
...
void VertexProgram::M4X3(Vector4f &dst, Vector4f &src0, const Src &src1)
{
	Vector4f row0 = fetchRegister(src1, 0);
	Vector4f row1 = fetchRegister(src1, 1);
	Vector4f row2 = fetchRegister(src1, 2);
...
void VertexProgram::M4X4(Vector4f &dst, Vector4f &src0, const Src &src1)
{
	Vector4f row0 = fetchRegister(src1, 0);
	Vector4f row1 = fetchRegister(src1, 1);
	Vector4f row2 = fetchRegister(src1, 2);
...
void VertexProgram::BREAKP(const Src &predicateRegister)
{
...
{
	Bool condition = (*Pointer<Byte>(data + OFFSET(DrawData,vs.b[boolRegister.index])) != Byte(0));
...
void VertexProgram::ENDLOOP()
{
	...
	aL[loopDepth] = aL[loopDepth] + increment[loopDepth];
...
void VertexProgram::LOOP(const Src &integerRegister)
{
	...
	iteration[loopDepth] = iteration[loopDepth] - 1;
