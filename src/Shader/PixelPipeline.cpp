...
void PixelPipeline::blendTexture(Vector4s &temp, Vector4s &texture, int stage)
{
	...
	if(textureStage.stageOperation != TextureStage::STAGE_DOT3)
	{
		...
		switch(textureStage.firstModifierAlpha)
		{
		case TextureStage::MODIFIER_COLOR:
			break;
		case TextureStage::MODIFIER_INVCOLOR:
		...
		switch(textureStage.secondModifierAlpha)
		{
		case TextureStage::MODIFIER_COLOR:
			break;
		case TextureStage::MODIFIER_INVCOLOR:
		...
		switch(textureStage.thirdModifierAlpha)
		{
		case TextureStage::MODIFIER_COLOR:
			break;
		case TextureStage::MODIFIER_INVCOLOR:
...
{
	Int kill = SignMask(CmpNLT(u, Float4(0.0f))) & SignMask(CmpNLT(v, Float4(0.0f))) & SignMask(CmpNLT(s, Float4(0.0f)));
...
{
	if(component == 0 || previousScaling != signedScaling)
	{
		U = Float4(src0.x);
		V = Float4(src0.y);
		W = Float4(src0.z);
...
void PixelPipeline::CND(Vector4s &dst, Vector4s &src0, Vector4s &src1, Vector4s &src2)
{
	...
	t0 = src0.x; t0 = CmpGT(t0, Short4(0x0800));
	t1 = src1.x; t1 = t1 & t0;
	t0 = ~t0 & src2.x; t0 = t0 | t1;
	dst.x = t0;

	t0 = src0.y; t0 = CmpGT(t0, Short4(0x0800));
	t1 = src1.y; t1 = t1 & t0;
	t0 = ~t0 & src2.y; t0 = t0 | t1;
	dst.y = t0;

	t0 = src0.z; t0 = CmpGT(t0, Short4(0x0800));
	t1 = src1.z; t1 = t1 & t0;
	t0 = ~t0 & src2.z; t0 = t0 | t1;
	dst.z = t0;

	t0 = src0.w; t0 = CmpGT(t0, Short4(0x0800));
	t1 = src1.w; t1 = t1 & t0;
	t0 = ~t0 & src2.w; t0 = t0 | t1;
	dst.w = t0;
}
...
void PixelPipeline::CMP(Vector4s &dst, Vector4s &src0, Vector4s &src1, Vector4s &src2)
{
	...
	t0 = CmpGT(Short4(0x0000), src0.x);
	t1 = src2.x; t1 &= t0;
	t0 = ~t0 & src1.x; t0 |= t1;
	dst.x = t0;

	t0 = CmpGT(Short4(0x0000), src0.y);
	t1 = src2.y; t1 &= t0;
	t0 = ~t0 & src1.y; t0 |= t1;
	dst.y = t0;

	t0 = CmpGT(Short4(0x0000), src0.z);
	t1 = src2.z; t1 &= t0;
	t0 = ~t0 & src1.z; t0 |= t1;
	dst.z = t0;

	t0 = CmpGT(Short4(0x0000), src0.w);
	t1 = src2.w; t1 &= t0;
	t0 = ~t0 & src1.w; t0 |= t1;
	dst.w = t0;
}
...
void PixelPipeline::BEM(Vector4s &dst, Vector4s &src0, Vector4s &src1, int stage)
{
	...
	t0 = MulHigh(src1.x, *Pointer<Short4>(data + OFFSET(DrawData,textureStage[stage].bumpmapMatrix4W[0][0]))); t0 = t0 << 4;
	t1 = MulHigh(src1.y, *Pointer<Short4>(data + OFFSET(DrawData,textureStage[stage].bumpmapMatrix4W[1][0]))); t1 = t1 << 4;
	t0 = AddSat(t0, t1);
	t0 = AddSat(t0, src0.x);
	dst.x = t0;

	t0 = MulHigh(src1.x, *Pointer<Short4>(data + OFFSET(DrawData,textureStage[stage].bumpmapMatrix4W[0][1]))); t0 = t0 << 4;
	t1 = MulHigh(src1.y, *Pointer<Short4>(data + OFFSET(DrawData,textureStage[stage].bumpmapMatrix4W[1][1]))); t1 = t1 << 4;
	t0 = AddSat(t0, t1);
	t0 = AddSat(t0, src0.y);
	dst.y = t0;
}
