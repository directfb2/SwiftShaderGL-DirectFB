...
Float4 sine_pi(RValue<Float4> x, bool pp = false);
Float4 cosine_pi(RValue<Float4> x, bool pp = false);
...
Float4 arccosh(RValue<Float4> x, bool pp = false);
Float4 arcsinh(RValue<Float4> x, bool pp = false);
Float4 arctanh(RValue<Float4> x, bool pp = false);
...
public:
	...
	const Vector4f operator[](RValue<Int4> i);
...
template <> struct PrintValue::Ty<sw::Vector4f>
{
	...
	static std::vector<Value*> val(const sw::Vector4f& v)
	{
...
template <> struct PrintValue::Ty<sw::Vector4s>
{
	...
	static std::vector<Value*> val(const sw::Vector4s& v)
	{
		return PrintValue::vals(v.x, v.y, v.z, v.w);
	}
};
