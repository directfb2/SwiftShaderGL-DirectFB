...
void VertexPipeline::pipeline(UInt &index)
{
	...
	else
	{
		...
		Vector4f ambient;
		Float4 globalAmbient = *Pointer<Float4>(data + OFFSET(DrawData,ff.globalAmbient));
		...
		for(int i = 0; i < 8; i++)
		{
			...
			// Attenuation
			{
				...
				L.x = L.y = L.z = *Pointer<Float4>(data + OFFSET(DrawData,ff.lightPosition[i]));
				L.x = L.x.xxxx;
				L.y = L.y.yyyy;
				L.z = L.z.zzzz;
			...
			// Ambient per light
			{
				Float4 lightAmbient = *Pointer<Float4>(data + OFFSET(DrawData,ff.lightAmbient[i]));
			...
			if(state.vertexNormalActive)
			{
				...
				{
					diff.x = diff.y = diff.z = *Pointer<Float4>(data + OFFSET(DrawData,ff.materialDiffuse));
					diff.x = diff.x.xxxx;
					diff.y = diff.y.yyyy;
					diff.z = diff.z.zzzz;
				}
				...
				o[C0].x = o[C0].x + diff.x * dot * lightDiffuse.x;
				o[C0].y = o[C0].y + diff.y * dot * lightDiffuse.y;
				o[C0].z = o[C0].z + diff.z * dot * lightDiffuse.z;
			}
			...
			if(state.vertexSpecularActive)
			{
				...
				Float4 dot = Max(dot3(C, normal), Float4(0.0f));
				...
				{
					Float4 materialSpecular = *Pointer<Float4>(data + OFFSET(DrawData,ff.materialSpecular));
		...
		{
			Float4 materialAmbient = *Pointer<Float4>(data + OFFSET(DrawData,ff.materialAmbient));
		...
		{
			Float4 materialEmission = *Pointer<Float4>(data + OFFSET(DrawData,ff.materialEmission));
		...
		if(state.vertexDiffuseMaterialSourceActive == MATERIAL_MATERIAL)
		{
			o[C0].w = Float4(*Pointer<Float4>(data + OFFSET(DrawData,ff.materialDiffuse[0]))).wwww;
		}
		else if(state.vertexDiffuseMaterialSourceActive == MATERIAL_COLOR1)
		{
		...
		if(state.vertexSpecularActive)
		{
			...
			if(state.vertexSpecularMaterialSourceActive == MATERIAL_MATERIAL)
			{
				o[C1].w = Float4(*Pointer<Float4>(data + OFFSET(DrawData,ff.materialSpecular[3]))).wwww;
			}
			else if(state.vertexSpecularMaterialSourceActive == MATERIAL_COLOR1)
			{
				Vector4f alpha = v[Color0];
	...
	if(state.fogActive)
	{
		...
		{
			f = Sqrt(dot3(vertexPosition, vertexPosition));
		}
...
void VertexPipeline::processTextureCoordinate(int stage, Vector4f &normal, Vector4f &position)
{
	if(state.output[T0 + stage].write)
	{
		...
		case TEXGEN_REFLECTION:
			{
				...
				if(state.vertexNormalActive)
				{
					Vector4f Nc = normal; // Normal vector in camera space
		...
		{
		case 4:
			texTrans3.x = texTrans3.y = texTrans3.z = texTrans3.w = *Pointer<Float4>(data + OFFSET(DrawData,ff.textureTransform[stage][3]));
		...
		case 3:
			texTrans2.x = texTrans2.y = texTrans2.z = texTrans2.w = *Pointer<Float4>(data + OFFSET(DrawData,ff.textureTransform[stage][2]));
		...
		case 2:
			texTrans1.x = texTrans1.y = texTrans1.z = texTrans1.w = *Pointer<Float4>(data + OFFSET(DrawData,ff.textureTransform[stage][1]));
		...
		case 1:
			texTrans0.x = texTrans0.y = texTrans0.z = texTrans0.w = *Pointer<Float4>(data + OFFSET(DrawData,ff.textureTransform[stage][0]));
...
void VertexPipeline::processPointSize()
{
	...
	if(state.pointScaleActive && !state.preTransformed)
	{
		...
		Float4 d = Sqrt(dot3(p, p));

		Float4 A = *Pointer<Float>(data + OFFSET(DrawData,point.pointScaleA));
		Float4 B = *Pointer<Float>(data + OFFSET(DrawData,point.pointScaleB));
		Float4 C = *Pointer<Float>(data + OFFSET(DrawData,point.pointScaleC));

		A = RcpSqrt_pp(A + d * (B + d * C));

		o[Pts].y = o[Pts].y * Float4(*Pointer<Float>(data + OFFSET(DrawData,viewportHeight))) * A;
	}
}
