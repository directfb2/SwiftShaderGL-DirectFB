...
std::string Shader::Instruction::string(ShaderType shaderType, unsigned short version) const
{
	...
	if(opcode != OPCODE_DCL)
	{
		...
		{
			instructionString += " " + dst.string(shaderType, version) + dst.relativeString() + dst.maskString();
		}
		...
		for(int i = 0; i < 4; i++)
		{
			if(src[i].type != PARAMETER_VOID)
			{
				...
				instructionString += src[i].preModifierString() + src[i].string(shaderType, version) + src[i].relativeString() + src[i].postModifierString() + src[i].swizzleString();
			}
		}
	}
	else
...
{
	return predicate || analysisBranch || analysisBreak || analysisContinue || analysisLeave;
}
...
void Shader::analyzeDynamicBranching()
{
	...
	for(unsigned int i = 0; i < instruction.size(); i++)
	{
		...
		if(breakDepth > 0)
		{
			if(instruction[i]->isLoop() || instruction[i]->opcode == OPCODE_SWITCH)
			{
				breakDepth++;
			}
...
// nesting of control flow blocks and function calls.
// These calculations are stored into the limits member, and are used by the
// programs to allocate stack storage variables.
void Shader::analyzeLimits()
{
	...
	struct FunctionInfo
	{
		...
		std::unordered_set<FunctionID> calls;
		bool reachable;
	};
	...
	// Assert that every function is reachable.
