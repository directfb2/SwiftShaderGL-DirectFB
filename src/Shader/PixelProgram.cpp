...
extern bool halfIntegerCoordinates;
...
Vector4f PixelProgram::readConstant(const Src &src, unsigned int offset)
{
	...
	else
	{
		...
		case Shader::PARAMETER_MISCTYPE:
			...
			{
			case Shader::VPosIndex:
				a = vPos.x;
				break;
			case Shader::VFaceIndex:
				a = vFace.x;
				break;
			default:
				ASSERT(false);
...
Float4 PixelProgram::linearToSRGB(const Float4 &x)
{
	// Approximates x^(1.0/2.2)
...
void PixelProgram::BREAKP(const Src &predicateRegister)
{
...
{
	Bool condition = (*Pointer<Byte>(data + OFFSET(DrawData,ps.b[boolRegister.index])) != Byte(0));
