...
{
	this->textureFilter = (FilterType)std::min(textureFilter, maximumTextureFilterQuality);
}

void Sampler::setMipmapFilter(MipmapType mipmapFilter)
{
	mipmapFilterState = (MipmapType)std::min(mipmapFilter, maximumMipmapFilterQuality);
}
...
FilterType Sampler::getTextureFilter() const
{
	...
	{
		return (FilterType)std::min(filter, FILTER_LINEAR);
	}
