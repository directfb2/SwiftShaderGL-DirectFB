...
inline Color<float> Surface::Buffer::read(void *element) const
{
	...
	case FORMAT_A8:
		...
		break;
	case FORMAT_R8_SNORM:
		r = std::max((*(signed char*)element) * (1.0f / 0x7F), -1.0f);
		break;
	case FORMAT_R8:
	...
	case FORMAT_A8B8G8R8_SNORM:
		{
			...
			r = std::max(abgr[0] * (1.0f / 0x7F), -1.0f);
			g = std::max(abgr[1] * (1.0f / 0x7F), -1.0f);
			b = std::max(abgr[2] * (1.0f / 0x7F), -1.0f);
			a = std::max(abgr[3] * (1.0f / 0x7F), -1.0f);
		}
		break;
	case FORMAT_A8B8G8R8:
	case FORMAT_SRGB8_A8:
	...
	case FORMAT_X8B8G8R8_SNORM:
		{
			...
			r = std::max(bgr[0] * (1.0f / 0x7F), -1.0f);
			g = std::max(bgr[1] * (1.0f / 0x7F), -1.0f);
			b = std::max(bgr[2] * (1.0f / 0x7F), -1.0f);
		}
		break;
	case FORMAT_X8B8G8R8:
	case FORMAT_SRGB8_X8:
	...
	case FORMAT_Q16W16V16U16:
		...
		break;
	case FORMAT_L8:
		r = g = b = *(unsigned char*)element * (1.0f / 0xFF);
		break;
	case FORMAT_A4L4:
		{
			unsigned char al = *(unsigned char*)element;

			r = g = b = (al & 0x0F) * (1.0f / 0x0F);
			a = (al & 0xF0) * (1.0f / 0xF0);
		}
		break;
	case FORMAT_L16:
		r = g = b = *(unsigned short*)element * (1.0f / 0xFFFF);
		break;
	case FORMAT_A8L8:
		r = g = b = ((unsigned char*)element)[0] * (1.0f / 0xFF);
		a = ((unsigned char*)element)[1] * (1.0f / 0xFF);
		break;
	case FORMAT_L16F:
		r = g = b = *(half*)element;
...
void *Surface::lockStencil(int x, int y, int front, Accessor client)
{
	...
	return stencil.lockRect(x, y, front, LOCK_READWRITE);
}
...
void Surface::update(Buffer &destination, Buffer &source)
{
	if(destination.buffer != source.buffer)
	{
		...
		{
		case FORMAT_R8G8B8:                         decodeR8G8B8(destination, source);         break;
		case FORMAT_X1R5G5B5:                       decodeX1R5G5B5(destination, source);       break;
		case FORMAT_A1R5G5B5:                       decodeA1R5G5B5(destination, source);       break;
		case FORMAT_X4R4G4B4:                       decodeX4R4G4B4(destination, source);       break;
		case FORMAT_A4R4G4B4:                       decodeA4R4G4B4(destination, source);       break;
		case FORMAT_P8:                             decodeP8(destination, source);             break;
		case FORMAT_DXT1:                           decodeDXT1(destination, source);           break;
		case FORMAT_DXT3:                           decodeDXT3(destination, source);           break;
		case FORMAT_DXT5:                           decodeDXT5(destination, source);           break;
		case FORMAT_ATI1:                           decodeATI1(destination, source);           break;
		case FORMAT_ATI2:                           decodeATI2(destination, source);           break;
		case FORMAT_R11_EAC:                        decodeEAC(destination, source, 1, false);  break;
		case FORMAT_SIGNED_R11_EAC:                 decodeEAC(destination, source, 1, true);   break;
		case FORMAT_RG11_EAC:                       decodeEAC(destination, source, 2, false);  break;
		case FORMAT_SIGNED_RG11_EAC:                decodeEAC(destination, source, 2, true);   break;
		case FORMAT_ETC1:
		case FORMAT_RGB8_ETC2:                      decodeETC2(destination, source, 0, false); break;
		case FORMAT_SRGB8_ETC2:                     decodeETC2(destination, source, 0, true);  break;
		case FORMAT_RGB8_PUNCHTHROUGH_ALPHA1_ETC2:  decodeETC2(destination, source, 1, false); break;
		case FORMAT_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2: decodeETC2(destination, source, 1, true);  break;
		case FORMAT_RGBA8_ETC2_EAC:                 decodeETC2(destination, source, 8, false); break;
		case FORMAT_SRGB8_ALPHA8_ETC2_EAC:          decodeETC2(destination, source, 8, true);  break;
...
void Surface::genericUpdate(Buffer &destination, Buffer &source)
{
	...
	int depth = std::min(destination.depth, source.depth);
	int height = std::min(destination.height, source.height);
	int width = std::min(destination.width, source.width);
...
{
	ETC_Decoder::Decode((const byte*)external.lockRect(0, 0, 0, LOCK_READONLY), (byte*)internal.lockRect(0, 0, 0, LOCK_UPDATE), external.width, external.height, internal.width, internal.height, internal.pitchB, internal.bytes, (nbAlphaBits == 8) ? ETC_Decoder::ETC_RGBA : ((nbAlphaBits == 1) ? ETC_Decoder::ETC_RGB_PUNCHTHROUGH_ALPHA : ETC_Decoder::ETC_RGB));
	...
	default:
		{
			...
			// Unpacking 4 bytes to short4 in the sampler currently involves reading 8 bytes,
...
void Surface::copyCubeEdge(Edge dstEdge, Surface *src, Edge srcEdge)
{
	...
	// copying top <-> right or bottom <-> left:
	//
	//      | +y |
	// | -x | +z | +x | -z |
	//      | -y |
...
Format Surface::selectInternalFormat(Format format) const
{
	...
	case FORMAT_G16R16F:
		return FORMAT_G32R32F;
	case FORMAT_B16G16R16F:
		return FORMAT_X32B32G32R32F;
	case FORMAT_X16B16G16R16F:
		return FORMAT_X32B32G32R32F;
	case FORMAT_A16B16G16R16F:
		return FORMAT_A32B32G32R32F;
	case FORMAT_X16B16G16R16F_UNSIGNED:
		return FORMAT_X32B32G32R32F_UNSIGNED;
	case FORMAT_A32F:
		return FORMAT_A32B32G32R32F;
	case FORMAT_R32F:
		return FORMAT_R32F;
	...
	// YUV formats
	case FORMAT_YV12_BT601:
		return FORMAT_YV12_BT601;
	case FORMAT_YV12_BT709:
		return FORMAT_YV12_BT709;
	case FORMAT_YV12_JFIF:
			...
			if(internal.samples == 2)
			{
				for(int y = 0; y < height; y++)
				{
					for(int x = 0; x < width; x += 8)
					{
						...
						static const word8 r_b = {0xF81F, 0xF81F, 0xF81F, 0xF81F, 0xF81F, 0xF81F, 0xF81F, 0xF81F};
						static const word8 _g_ = {0x07E0, 0x07E0, 0x07E0, 0x07E0, 0x07E0, 0x07E0, 0x07E0, 0x07E0};
