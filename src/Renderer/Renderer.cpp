...
extern bool symmetricNormalizedDepth;
...
void Renderer::draw(DrawType drawType, unsigned int indexOffset, unsigned int count, bool update)
{
	...
	for(int q = 0; q < ss; q++)
	{
		...
		if(pixelState.transparencyAntialiasing == TRANSPARENCY_ALPHA_TO_COVERAGE)
		{
			...
			float margin = std::min(ref, 1.0f - ref);
	...
	case Task::PRIMITIVES:
		{
			...
			TRACE("vertexTime[%d] = %lld", threadIndex, vertexTime[threadIndex]);
			startTick = time;
			#endif
			...
			TRACE("setupTime[%d] = %lld", threadIndex, setupTime[threadIndex]);
			#endif
		}
		break;
	case Task::PIXELS:
		{
			...
			TRACE("pixelTime[%d] = %lld", threadIndex, pixelTime[threadIndex]);
			#endif
		}
		break;
	case Task::RESUME:
...
bool Renderer::setupPoint(Primitive &primitive, Triangle &triangle, const DrawCall &draw)
{
	...
	if((C[0] & C[1] & C[2] & C[3]) == Clipper::CLIP_FINITE)
	{
		...
		triangle.v2.Y -= iround(16 * 0.5f * pSize) * (data.Hx16[0] > 0.0f ? 1 : -1);
		return setupRoutine(&primitive, &triangle, &polygon, &data);
	}
...
void Renderer::updateConfiguration(bool initialUpdate)
{
	...
	if(newConfiguration || initialUpdate)
	{
		...
		{
			if(pass != Optimization::Pass::Disabled) { cfg.add(pass); }
		}

		Nucleus::adjustDefaultConfig(cfg);
